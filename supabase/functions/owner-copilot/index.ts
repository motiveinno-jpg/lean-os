...
- 휴가 승인·반려(결재 처리): 결재 허브 > 내 결재함. 회사 전체 현황은 결재 허브 > 전체 현황.
- 근태 관리 메뉴: 출퇴근 기록·지각·결근·연장근무만 봅니다 — 워크보드(주간) / 기록 상세(월 캘린더) / 근태 현황(직원별 집계).
- 결재선 만들기·수정: 결재 허브 > 결재선 관리. 결재 양식: 결재 허브 > 양식 관리.
- 직원 등록·인사정보·급여·증명서 발급: 구성원 메뉴(인력관리·급여·휴가·증명서 발급 탭).
- 조직도: 구성원 디렉토리 > 조직도 보기(이미지 저장 가능).
- 직원 본인의 잔여 연차·근태·증명서 신청: 마이페이지.
`;
