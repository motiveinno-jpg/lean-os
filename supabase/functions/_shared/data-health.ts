...
//   모든 조회를 이 함수에 통과시킨다. 실패하면 던져서 바깥 catch 가 failed 로 센다.
function ok<T extends { data: unknown; error: unknown }>(res: T): T {
...
export async function collectDataHealth(admin: Admin, companyId: string): Promise<HealthReport> {
  ...
  try {
    const { data } = ok(await admin.from("employees")
  ...
  try {
    const { data } = ok(await admin.from("leave_balances")
    ...
    if (rows.length) {
      const { data: emps } = ok(await admin.from("employees").select("id, name").eq("company_id", companyId));
  ...
  try {
    const { data } = ok(await admin.from("attendance_records")
  ...
  try {
    const { data } = ok(await admin.from("signature_requests")
  ...
  try {
    const { data } = ok(await admin.from("approval_requests")
  ...
  try {
    const { data } = ok(await admin.from("tax_invoices")
