-- 정산 자동 전표(차액 잡손익 포함)를 거래처원장에서 수정 허용 (2026-06-17)
--   회사마다 차액 계정(잡손실→이자·수수료 등)이 달라 수동 수정 필요.
--   update_manual_voucher 의 source 가드를 완화: source='manual' 또는 정산연결 전표(rule+linked_settlement_id).
--   나머지 로직(균형·마감월·이력·재작성)은 동일. 함수 본문 ASCII.
...
begin
  ...
  if v_e.source <> 'manual' and not (v_e.source = 'rule' and v_e.linked_settlement_id is not null) then
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    if v_acct is null or not exists (select 1 from chart_of_accounts a where a.id = v_acct and a.company_id = v_company) then raise exception 'INVALID_ACCOUNT'; end if;
    if coalesce(v_line->>'bank_account_id', '') <> '' and not exists (select 1 from bank_accounts b where b.id = (v_line->>'bank_account_id')::uuid and b.company_id = v_company) then raise exception 'INVALID_BANK_ACCOUNT'; end if;
    if coalesce(v_line->>'card_id', '') <> '' and not exists (select 1 from corporate_cards c where c.id = (v_line->>'card_id')::uuid and c.company_id = v_company) then raise exception 'INVALID_CARD'; end if;
    v_debit := v_debit + v_d;
  ...
  select jsonb_build_object(
    'entry_date', v_e.entry_date, 'description', v_e.description,
  ...
  set entry_date = p_entry_date, description = coalesce(p_description, description),
