-- 수동 전표 수정 RPC 안전판 강화 (2026-06-16 거래처원장 수정 핸드오프 §1-1)
--   기존 update_manual_voucher 는 status='confirmed' 만 검사 → 승인된 rule/ai 전표도 수정 가능했음.
--   거래처 원장에서 수정 가능한 것은 source='manual' 전표뿐이므로 DB 레벨에서도 차단(프론트 가드 보조).
