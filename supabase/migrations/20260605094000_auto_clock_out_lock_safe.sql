-- 자동 퇴근 cron 락 경합 → 커넥션 슬롯 고갈(504) 방지 (2026-06-05 인시던트)
--
-- 증상: auto-clock-out(5분 cron)이 퇴근시간대(18시 KST)에 23초~4분씩 실패 반복.
--   원인: 라이브 사용자의 수동 퇴근이 잡은 attendance_records 행을 cron UPDATE 가 기다림.
--   pg_cron 은 postgres 역할이라 lock_timeout/statement_timeout 무제한 → 분 단위 hang,
--   5분마다 누적 → 직접 커넥션 슬롯 고갈 → DB 다운(전 페이지 504).
--
-- 수정:
--   1) pg_try_advisory_xact_lock — 이전 실행이 안 끝났으면 즉시 skip(겹침 누적 차단)
--   2) SET LOCAL lock_timeout/statement_timeout — 단일 실행이 끌지 않게 안전망
--   3) FOR UPDATE SKIP LOCKED — 라이브가 잡은 행은 건너뛰고 다음 주기에 재처리(경합 제거)
-- 로직(누가 자동퇴근 대상인지)·알림 문구는 무변경.
