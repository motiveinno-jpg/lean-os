-- 정률법 월 상각액 = (취득가 − 누계) × 연 상각률 ÷ 12. 연 상각률은 세법 표(내용연수 n년: 1 − 0.05^(1/n), 소수 셋째 자리)와 같다.
--   종전 2/내용월수(월 이중체감)는 세법 상각률과 달라 세무사 패키지의 '정률' 표기와 어긋났다.
begin;
create or replace function public._fa_declining_rate(p_useful_months integer)
returns numeric language sql immutable as $$
  select case when coalesce(p_useful_months, 0) <= 0 then 0
              else round((1 - power(0.05, 12.0 / p_useful_months))::numeric, 3) end
$$;
