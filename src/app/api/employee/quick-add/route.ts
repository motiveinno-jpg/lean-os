...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const callerRow = logRead('quick-add/route:callerRow', await admin.from("users").select("id, company_id").eq("auth_id", caller.id).maybeSingle());
    ...
    if (emp?.id) {
      ...
      //   게다가 error 를 안 봐서 실패해도 "자동으로 직원 등록했습니다" 만 뜨고 연결은 안 됐다.
