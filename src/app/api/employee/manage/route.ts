...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (action === "remove-from-company") {
      ...
      //   former_company_id — 회사 구성원이 이 사람의 이름을 계속 읽을 수 있게(결재·게시글·파일 귀속). 없으면 '-' 로 보였다.
      const { error } = await admin.from("users")
        .update({ former_company_id: companyId, company_id: null, auth_id: null, email: `former+${userId}@removed.invalid` } as never)
        .eq("id", userId);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
