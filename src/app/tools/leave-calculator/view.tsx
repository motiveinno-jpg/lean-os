...
// 연차 계산기 화면 (2026-08-13) — 랜딩 v8(tl8-) 스타일 — 2026-09-14 lp4 에서 옮김.
      ...
      <section className="tl8-section tl8-bg-canvas">
        <div className="tl8-narrow">
          <div className="tl8-sec-head tl8-sec-head-c">
            ...
            <h1 className="tl8-h2">연차 계산기</h1>
            <p className="tl8-sub">입사일만 넣으면 근로기준법 제60조 기준으로 지금 발생한 연차를 계산해 드립니다. 회원가입 없이 무료입니다.</p>
          </div>
...
. onChange 가 input 호환이라 계산 로직은 그대로다. */}
              ...
              <label className="tl8-field">
                ...
                <DateField className="tl8-input" value={base} onChange={(e) => setBase(e.target.value)} />
              </label>
            </div>
            ...
            {result ? (
              <div className="tl8-result" aria-live="polite">
                ...
                <div className="tl8-result-rows">
                  {result.kind === "under1" && (
                    <div className="tl8-result-row">1년 미만은 <b>1개월 개근마다 1일</b>  (최대 11일). 개근을 가정한 값입니다</div>
                  )}
                  {result.kind === "over1" && (
                    <div className="tl8-result-row">1년 이상은 <b>15일 + 2년마다 1일 가산</b> (최대 25일), 전년 출근율 80% 이상 기준</div>
                  )}
                      ...
                      <table className="tl8-table tl8-table-tight">
                        ...
                        <tbody>
                          {result.acc.map((r) => (
                            <tr key={r.label}>
                              <td>{r.label}</td>
                              <td className="tl8-dim">{r.detail}</td>
                              <td>{r.days}일</td>
                            </tr>
                          ))}
                          <tr><td><b>합계</b></td><td className="tl8-dim">근속 {result.years}년 {result.months}개월</td><td><b>{result.accTotal}일</b></td></tr>
                        </tbody>
                      </table>
          ...
          <div className="tl8-tablewrap">
            <h2 className="tl8-h3">근속연수별 연차 발생 일수 (근로기준법 제60조)</h2>
          ...
          <p className="tl8-note">
            다른 무료 도구: <Link href="/tools/severance-calculator" className="tl8-crosslink">퇴직금 계산기</Link> · <Link href="/tools/insurance-calculator" className="tl8-crosslink">4대보험 계산기</Link> · <Link href="/tools/salary-calculator" className="tl8-crosslink">실수령액 계산기</Link> · <Link href="/tools/weekly-holiday-calculator" className="tl8-crosslink">주휴수당 계산기</Link> · <Link href="/tools/vat-calculator" className="tl8-crosslink">부가세 계산기</Link>
          </p>
        </div>
      </section>
      ...
      <section className="tl8-section tl8-bg-tint">
        <div className="tl8-narrow tl8-sec-head-c">
          <h2 className="tl8-h2">직원별 연차, 아직 엑셀로 세고 계신가요?</h2>
          <p className="tl8-sub">오너뷰는 입사일 기준·회계연도 기준 모두 매월 자동으로 부여하고, 신청·승인·잔여 관리까지 한 곳에서 끝냅니다. 카드 등록 없이 무료로 시작하세요.</p>
          <div className="tl8-feat-cta">
            <Link href="/auth?mode=signup" className="btn btn-fill" data-cta="signup:tool_leave">무료로 시작하기</Link>
