...
export default function MonthlyDetailPage() {
  ...
  const extreme = (it: Item) => { if (!view.hl.includes("minmax") || !(it.key === "revenue" || it.key === "op")) return { max: -1, min: -1 }; const vals = shownGroups.map((g) => valueOf(it, g.i)); if (!vals.some((v) => v !== 0)) return { max: -1, min: -1 }; let max = 0, min = 0; vals.forEach((v, i) => { if (v > vals[max]) max = i; if (v < vals[min]) min = i; }); return { max: shownGroups[max].i, min: shownGroups[min].i }; };
