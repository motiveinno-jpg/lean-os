...
export const ROW_TIPS: Record<string, { title: string; body: string }> = {
  incomeTotal: { title: "수입 총액", body: "그 달에 들어온 돈 전체입니다. 매출 + 보조금/지원금 + 대표 가수금 + 기타 수입을 모두 더한 값입니다." },
  salesRevenue: { title: "매출", body: "발행된 매출 세금계산서 기준 금액(공급가액 + 부가세)입니다. 실제 통장 입금이 아니라 '매출로 잡힌' 금액이라, 미수금이 있으면 통장 잔액과 차이가 날 수 있습니다." },
  ...
  expenseTotal: { title: "지출 총액", body: "그 달에 나간 돈 전체입니다. 고정비 + 변동비를 더한 값입니다." },
  fixedCosts: { title: "고정비", body: "매달 비슷하게 나가는 정기 지출입니다. 등록된 정기지출(구독·임차료 등)과 고정비 항목에서 자동 집계됩니다. 매출이 0이어도 나가는 비용입니다." },
  variableCosts: { title: "변동비", body: "매출·활동에 따라 달라지는 지출입니다. 정기지출이 아닌 지급건과 카드 사용액에서 집계됩니다." },
  vat: { title: "부가세 (분기 신고)", body: "그 분기의 매출세액에서 매입세액을 뺀 실제 납부(또는 환급) 예상액입니다. 신고하는 달(1·4·7·10월)에 몰아서 표기됩니다. +면 납부, −면 환급입니다." },
  netProfit: { title: "순이익 (수입−지출)", body: "그 달 수입 총액에서 지출 총액을 뺀 금액입니다. 회계상 세밀한 손익이 아니라 자금 기준으로 남은 금액입니다." },
  opMargin: { title: "영업이익률", body: "순이익 ÷ 수입 총액 × 100. 100원 벌어 몇 원이 남았는지 보는 비율입니다. 높을수록 수익성이 좋습니다." },
  ...
  gap: { title: "누적순익 − 통장 차액", body: "장부상 쌓인 누적 순이익과 실제 통장 잔액의 차이입니다. 차이가 크면 대표 가수금·미반영 거래·인출 등 장부에 안 잡힌 자금 이동이 있다는 신호이므로 점검이 필요합니다." },
