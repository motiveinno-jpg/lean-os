...
export default function PnlPage() {
  ...
  return (
    <div id="pnl-printable" style={{ padding: "24px 28px", maxWidth: 1400 }}>
      ...
      >
        <strong style={{ color: "var(--text-muted)" }}>📌 각 항목 출처</strong>
        <br />
        <strong>· 매출</strong>: <code>bank_transactions</code> 에서 <code>type='income'</code>(입금) 거래의 amount 합계.
        <br />
        <strong>· 기타수익</strong>: <code>tax_invoices</code> 매출 세금계산서(<code>type='매출'</code>) supply_amount 합계가 같은 달 은행 입금보다 클 때 그 차액. 외상 매출 / 미수금 / 입금 지연 분이 여기 잡힙니다. 큰 금액이라면 ① 매출 세금계산서를 발행했는데 아직 입금 안 됐거나 ② 세금계산서가 중복/잘못 등록됐을 가능성. 세금계산서 탭에서 매출 목록 점검 권장.
        <br />
        <strong>· 외주비 / 인프라비용 (매출원가)</strong>: 은행 출금 거래의 거래처·적요에 키워드 매칭(외주 / 인프라·서버·호스팅·AWS·클라우드·도메인). 사용자가 카테고리를 따로 입력해도 키워드가 있으면 매출원가로 분리.
        <br />
        <strong>· 판매관리비</strong>: 위 매출원가 외 모든 출금 거래. 매핑 모달에서 입력한 카테고리 값으로 행 그룹화. 카테고리 비어있으면 키워드(급여/임대료/소프트웨어/전문서비스/복리후생/4대보험)로 자동 분류, 그래도 안 잡히면 '기타'.
        <br />
        <strong>· 급여 / 4대보험</strong>: 직원 페이지 등록 정보 기반 자동 누적. 4대보험 = 급여 × 10.55% (국민 4.5 + 건강 3.545 + 장기요양 0.459 + 고용 1.35 + 산재 0.7%).
        <br />
        <strong>· 매입 세금계산서</strong>: 손익계산서에 직접 반영되지 않습니다(은행 출금이 이미 비용으로 들어감 → 이중 계상 방지). 매입 세금계산서는 부가세 신고용 + 재무상태표의 '미지급금'(status=received/issued/modified) 으로만 사용됩니다.
        <br />
        <br />
        💡 분류 정확도 ↑: 거래내역 매핑 모달에서 직접 카테고리를 입력하세요. 키워드 fallback 보다 사용자 입력이 우선.
      </div>
    </div>
  );
}
