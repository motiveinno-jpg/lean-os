          ...
          {tab === "import" && (
            <QueryBar>
              ...
              <span className="inv-hint">
                채널 주문 엑셀을 붙여 넣으면 <b>출고(판매)로 등록</b>됩니다.
                <b> 같은 주문번호는 중복 등록되지 않습니다.</b>
              </span>
            </QueryBar>
          )}
    ...
    <div className="ch-import">
      {codesCount === 0 && (
        <p className="inv-warn">
          {channelLabel(channel)}에 연결된 상품이 <b>없습니다</b>.
          붙여 넣은 줄은 모두 &lsquo;미연결&rsquo;로 처리됩니다.
          <button type="button" className="bz-link ch-link" onClick={goCodes}>상품 연결로 이동</button>
        </p>
      )}

      <div className="ch-grid">
        <label className="inv-field"><span>출고 창고 *</span>
          <select className="field-input" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)}>
            {warehouses.length === 0 && <option value="">창고 없음 (재고 › 창고에서 등록)</option>}
      ...
      <label className="inv-field"><span>주문 목록 (엑셀 붙여넣기)</span>
      ...
      <p className="inv-foot">
        열 순서: <b>주문번호 · 채널 상품코드 · 수량</b> (단가·주문일·주문자는 선택).
        엑셀에서 해당 열을 복사해 붙여 넣으세요.
      </p>
      ...
      <div className="ch-actions">
        ...
        <button type="button" className="btn-primary btn-sm"
          ...
          onClick={async () => {
            if (!window.confirm(`${sum!.ok}줄을 출고(판매)로 등록합니다. 진행할까요?`)) return;
            setBusy(true);
            try {
              const r = await importChannelOrders(companyId!, channel, warehouseId, rows!, { docDate }, userId);
              toast(`${r.docNo} · 주문 ${r.orders}건 · ${r.lines}줄 등록`, "success");
      ...
      {rows && sum && (
        <>
          <div className="ch-sum">
            <span className="ch-sum-ok">등록 가능 <b>{sum.ok}</b></span>
            {sum.already > 0 && <span className="ch-sum-skip">기존 등록 <b>{sum.already}</b></span>}
            {sum.noCode > 0 && <span className="ch-sum-bad">미연결 <b>{sum.noCode}</b></span>}
            {sum.noTrack > 0 && <span className="ch-sum-skip">수량 미관리 <b>{sum.noTrack}</b></span>}
          </div>
          <div className="stg-table-wrap">
            <table className="ev-table ev-lined table-inv-ch-check">
              <thead><tr><th>주문번호</th><th>채널 상품코드</th><th>수량</th><th>단가</th><th>처리</th></tr></thead>
              <tbody>
                {rows.map((r, i) => (
                  <tr key={`${r.channel_order_no}-${i}`} className={r.reason === "no-code" ? "inv-row-fix" : undefined}>
                    ...
                    <td className="tc">
                      {r.reason === "ok" ? <span className="inv-pill inv-pill-ok">등록</span>
                        : r.reason === "already" ? <span className="inv-pill inv-pill-warn">기존 등록 — 건너뜀</span>
                        : r.reason === "no-track" ? <span className="inv-pill inv-pill-warn">수량 미관리 품목 — 건너뜀</span>
                        : <span className="inv-pill inv-pill-danger">미연결 상품코드</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {sum.noCode > 0 && (
            <p className="inv-foot inv-foot-warn">
              <b>미연결 상품코드 {sum.noCode}줄은 등록되지 않습니다.</b>
              <b> 상품 연결</b>에서 해당 코드를 SKU에 연결한 뒤 다시 &lsquo;검증&rsquo;하세요.
              상품명으로 자동 대응하지 않는 이유 — 유사한 상품명이 잘못 대응되면 재고 오류로 이어집니다.
            </p>
          )}
        </>
      )}
      ...
      <p className="inv-foot">
        <b>API 자동 수집은 준비 중입니다.</b> 네이버 커머스API·쿠팡 윙 API 키는 <b>판매자가 직접 발급</b>받아야 합니다.
        발급 후 <b>회사 설정 › 연동·API 키</b>에 등록하면 이 화면에 자동 수집이 추가됩니다.
        그때까지는 엑셀 붙여넣기로 동일하게 처리할 수 있습니다.
      </p>
    </div>
  );
}
