        ...
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr className="text-xs text-[var(--text-dim)] border-b border-[var(--border)]">
              ...
              {cols.map((c) => (
                <th key={c.key} className={`px-3 py-2 font-medium whitespace-nowrap ${c.type === "text" ? "text-center" : "text-center"} ${colWidth(c)}`}>{c.label}</th>
              ))}
...
function ColumnEditor({ cols, onClose, onSave }: { cols: QuoteCol[]; onClose: () => void; onSave: (cols: QuoteCol[]) => void }) {
  ...
  return (
    <div className="column-editor-modal fixed inset-0">
