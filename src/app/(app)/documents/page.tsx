...
function DocumentsPageInner() {
  ...
  const [userId, setUserId] = useState<string | null>(null);
  const [tab, setTab] = useState<"docs" | "invoices">("docs");
  ...
  const createDocMut = useMutation({
    mutationFn: () => createBlankDocument({
      companyId: companyId!, dealId: docForm.deal_id || undefined,
      name: docForm.name, type: docForm.type, createdBy: userId!,
    }),
    onSuccess: () => { invalidate(); setShowDocForm(false); setDocForm({ name: "", type: "contract", deal_id: "" }); },
  });
        ...
        <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] overflow-hidden">
          {documents.length === 0 ? (
            <div className="p-16 text-center">
              ...
              <div className="text-lg font-bold mb-2">문서가 없습니다</div>
              <div className="text-sm text-[var(--text-muted)]">계약서, 견적서, 제안서를 생성하세요</div>
            </div>
          ) : (
