...
const FEATURES: GuideFeature[] = [
  ...
  },
  {
    ...
    keyFeatures: [
      ...
      '스스로 조회·추론 — 정해진 질문 유형이 아니어도 회사 데이터를 직접 찾아 답변',
  ...
  },
  {
    ...
    category: '운영',
    description:
      '정부 지원사업을 우리 회사 자료(업종·직원·재무)로 걸러서 추천합니다. 받을 수 있는 것만 골라 보여주고 예상 수령액까지 계산하므로, 공고를 일일이 뒤지지 않아도 됩니다.',
    route: '/support-programs',
    keyFeatures: [
      '맞춤 추천 — 회사 업종·규모·인사 정보로 자격을 자동 판정',
      '예상 수령액 — 고용장려금 등 받을 수 있는 금액 계산',
      '대표·관리자 전용 — 직원 인사 정보를 읽으므로 권한자만 접근',
    ],
  },
  {
    id: 'inventory',
    ...
    category: '재고',
    description:
      '물건의 흐름을 관리합니다. 품목을 등록하고 창고관리에서 현재고를 보며, 주문 → 판매·구매·생산으로 재고가 움직입니다. 이커머스 채널 연동과 현황·이익관리(선입선출 원가)까지 재고 그룹 하나에서 해결합니다.',
    route: '/inventory/products',
    keyFeatures: [
      '품목 — SKU·규격·판매가·매입가 등록',
      '창고관리 — 현재고 확인과 입·출고·조정',
      '주문 — 주문서·견적 관리 (약속 단계라 재고는 안 움직임)',
      '판매·구매·생산 — 주문을 불러와 재고를 실제로 움직이는 세 갈래',
      '이커머스 — 온라인 주문 가져오기, 채널 상품 연결, 출고 처리',
      '현황·이익관리 — 집계 그래프와 선입선출 원가 반영 이익률',
    ],
    tips: '주문은 약속이라 재고를 건드리지 않습니다 — 판매·구매·생산이 주문을 불러와 재고를 움직입니다.',
  },
  {
    id: 'partners',
    icon: '🏢',
  ...
  },
  {
    ...
    keyFeatures: [
      ...
      '전자계산서·현금영수증 — 발행과 내역 관리 (세금·증빙 메뉴 안)',
      '발행 알림 메일 — 발행이 완료되면 지정한 주소로 자동 통보',
      '부가세 집계 — 신고용 집계는 분석 › 부가세에서 확인',
    ],
    tips: '설정 › 연동·API 키 › 은행연동에서 공동인증서를 등록하면 자동 수집이 시작됩니다.',
  },
  {
    id: 'collect',
    icon: '📥',
    ...
    category: '재무',
    description:
      '통장·카드·홈택스 수집을 한 판에서 보고, 수집된 거래를 장부(전표)로 만듭니다. 거래 매칭, 자동분개 규칙, 차·대 계정 직접 지정, 전표 취소까지 — 재무제표는 여기서 확정한 전표만 반영됩니다.',
    route: '/collect',
    keyFeatures: [
      '수집 한 판 — 통장·카드·홈택스 자료를 탭 하나에서 확인',
      '자동 분류 — 거래 내용 기반 계정과목·부가세 구분 자동 매핑, 수동 보정 시 학습',
      '전표 만들기 — 거래를 계정과목 분개로 확정 (한 줄 = 전표 한 장)',
      '거래 매칭 — 세금계산서·입금을 자동/수동으로 연결',
      '자동분개 규칙 — 반복 거래는 규칙으로 올려 자동 처리',
      '수기 기장 — 현금 등 수집 밖 거래는 일반전표·매입매출전표 메뉴에서 직접 입력',
      '전표 취소 — 잘못 만든 전표는 되돌리기로 정정',
    ],
    tips: '손익계산서·재무상태표에는 여기서 확정한 전표만 반영됩니다 — 수집만 하고 전표를 안 만들면 재무제표가 비어 보입니다.',
  },
  {
    id: 'assets',
    icon: '🏗️',
    ...
    category: '재무',
    description:
      '장비·차량·소프트웨어 같은 고정자산을 등록하면 매달 감가상각 전표 초안이 자동으로 만들어집니다. 취득부터 상각까지 자산 장부를 따로 계산할 필요가 없습니다.',
    route: '/finance/assets',
    keyFeatures: [
      '자산 등록 — 취득가·내용연수 등 자산 정보 관리',
  ...
  },
  {
    ...
    category: '재무',
    description:
      '원천세·부가세 신고서를 장부와 급여 데이터 기준으로 완성합니다. 오너뷰가 신고서를 채워 주고, 제출만 홈택스에서 직접 하면 됩니다.',
    route: '/finance/tax-filing',
    keyFeatures: [
      '원천세 — 급여 데이터 기반 신고서 작성',
      '부가세 — 매출·매입 증빙 집계로 신고서 완성',
      '홈택스 제출 — 완성본을 보고 그대로 옮겨 제출 (제출은 사람이)',
    ],
  },
  {
    id: 'reports',
    ...
    category: '재무',
    description:
      '수집된 거래와 장부를 바탕으로 회사의 재무 상태를 봅니다. 경영 요약 → 손익 현황 → 자금 전망 → 회계 자료 → 거래처 원장 → 부가세까지, 사이드바 분석 그룹의 메뉴로 나뉘어 있습니다.',
    route: '/reports/summary',
    keyFeatures: [
      '경영 요약 — 회사 재무 핵심 지표를 한 화면에',
      '손익 현황 — 월/분기 단위 매출·비용 추이와 계정과목별 집계',
      '자금 전망 — 예정 지출과 자금 흐름 시나리오',
      '회계 자료 — 손익계산서·재무상태표, 원장 드릴다운(전표번호·거래처·적요)',
  ...
  },
  {
    ...
    category: '재무',
    description:
      '여러 은행에 흩어진 법인 통장 잔액을 한 화면에 모아 보고, 법인카드 승인내역과 매달 나가는 정기 지출(구독·고정비)을 관리합니다. 재무 그룹의 통장/카드/정기 지출 메뉴로 나뉘어 있습니다.',
    route: '/bank',
    keyFeatures: [
  ...
  },
  {
    ...
    category: '영업',
    description:
      '프로젝트(거래) 단위로 돈의 흐름을 추적합니다. 견적 → 계약 → 전자서명 → 세금계산서 → 입금까지 한 흐름으로 잇고, 통장·카드·전표·결재 같은 증빙을 프로젝트에 연결해 프로젝트별 손익과 미수금을 관리합니다.',
    route: '/projecthub',
    keyFeatures: [
      '견적·계약·서명 체인 — 프로젝트 안에서 견적서를 보내고 계약·전자서명까지 연결',
      '거래처 왕복 — 거래처가 링크에서 수정 요청을 보내면 확인 후 재발송',
      '판매전표 자동 발행 — 계약 양측 서명 시 자동 기장 (설정 › 계정과목·분류에서 매출 계정 지정 시)',
      '증빙 연결 — 통장·카드·계산서·전표·결재를 프로젝트에 이어 보기, 연결 제안',
      '프로젝트 손익·미수금 — 수입·지출·마진 집계와 미입금 추적',
  ...
  },
  {
    ...
    keyFeatures: [
      ...
      '결재 정책 — 유형·금액별 결재선(단계·승인자·참조) 설정',
      '메일 알림 — 승인 차례인 사람에게 자동 발송, 총책임자 통보 주소도 지정 가능 (설정 > 회사정보)',
  ...
  },
  {
    ...
    category: '운영',
    description:
      '회사 문서와 파일을 한곳에 보관합니다. 전자계약 완료 문서부터 팀 공유 자료까지 폴더로 정리하고, 삭제는 기본적으로 본인이 올린 파일만 가능해 안전합니다.',
    route: '/documents',
    keyFeatures: [
      '파일 보관 — 폴더 구조로 회사 문서 정리',
      '전자계약 연계 — 서명 완료 문서를 보관함에서 관리',
      '안전한 삭제 — 남의 파일 삭제는 별도 권한이 있어야 가능',
    ],
  },
  {
    id: 'team',
    ...
    category: '운영',
    description:
      '전 구성원이 볼 수 있는 사내 주소록입니다. 동료의 부서·직급·연락처를 빠르게 찾을 수 있고, 연봉 같은 인사 정보는 보이지 않습니다.',
    route: '/team',
    keyFeatures: [
      '주소록 — 이름·부서·직급·연락처 조회',
  ...
  },
  {
    ...
    category: '운영',
    description:
      '사이드바 설정 그룹의 다섯 메뉴 — 회사 기초정보, 구성원·초대, 회계·세무, 연동·API 키, 보안·시스템 — 에서 서비스 전반을 설정합니다. 요금제 메뉴에서 구독 플랜과 결제를 관리합니다.',
    route: '/settings/company',
    keyFeatures: [
      '회사 기초정보 — 사업자 정보·직인·회사 양식·결재 총괄 알림',
      '구성원·초대 — 부서와 계정 초대 관리',
      '회계·세무 — 자금·통장, 계정과목·분류, 회계마감, 세무 파트너, 4대보험 요율',
      '연동·API 키 — 공동인증서(은행연동) 등록으로 금융·홈택스 자동 수집',
      '보안·시스템 — 접속 보안 등 시스템 설정',
      '요금제 — 구독 플랜 변경, 월간/연간 결제 관리',
    ],
    tips: '공동인증서 등록이 자동화의 시작점입니다. 설정 › 연동·API 키 › 은행연동에서 등록하세요.',
  },
];
...
const WORKFLOWS: Workflow[] = [
  {
    ...
    steps: [
      ...
      { title: '회사 정보 확인', description: '설정 → 회사 기초정보에서 사업자등록번호, 대표자명, 주소를 확인·보완합니다. 세금계산서와 계약서 등 공식 문서에 사용됩니다.', route: '/settings/company' },
      { title: '통장 등록', description: '설정 → 회계·세무 → 자금·통장에서 법인 계좌를 등록하고 용도(운영/세금/급여/예비)를 지정합니다.', route: '/settings/finance' },
  ...
  },
  {
    ...
    steps: [
      { title: '설정 → 연동·API 키', description: '설정 → 연동·API 키의 은행연동에서 등록을 시작합니다.', route: '/settings/integration' },
  ...
  },
  {
    ...
    icon: '🏦',
    title: '수집 → 전표 → 재무제표',
    description: '인증서 연동으로 거래가 자동 수집되고, 전표로 확정하면 재무제표까지 이어집니다.',
    steps: [
      { title: '인증서 등록 (선행)', description: '위의 "공동인증서 등록" 가이드를 먼저 완료하세요. 설정 → 연동·API 키 → 은행연동에서 연동할 은행·카드사를 관리할 수 있습니다.', route: '/settings/integration' },
      { title: '수집 확인', description: '수집·전표에서 통장·카드·홈택스 자료가 자동으로 쌓이는 것을 확인합니다.', route: '/collect' },
      { title: '전표 만들기', description: '수집된 거래를 계정과목 분개로 확정합니다. 반복 거래는 자동분개 규칙으로 올려두면 다음부터 자동 처리됩니다.', route: '/collect' },
      { title: '수기 거래 입력', description: '자동 수집 밖의 거래(현금 등)는 전표입력에서 직접 기록합니다.', route: '/partners/reconciliation/voucher-entry' },
      { title: '재무제표 확인', description: '확정한 전표가 손익계산서·재무상태표에 반영됩니다. 손익 계정을 누르면 원장(전표번호·거래처·적요)까지 내려갑니다.', route: '/reports' },
    ],
  },
  {
    id: 'deal-to-payment',
    ...
    steps: [
      { title: '프로젝트 생성', description: '프로젝트 메뉴에서 새 프로젝트를 만듭니다. 프로젝트명, 거래처, 계약 금액을 입력하면 진행 흐름 추적이 시작됩니다.', route: '/projecthub' },
      { title: '견적서 작성·발송', description: '프로젝트의 매출 탭에서 견적서를 작성해 거래처에 링크로 보냅니다. 거래처가 수정 요청을 보내면 확인 후 재발송하는 왕복도 지원합니다.', route: '/projecthub' },
      { title: '계약 체결 — 전자서명', description: '견적이 수락되면 품목·금액이 계약 카드로 이어집니다. 서명 요청을 보내면 상대방은 링크로 접속해 그 자리에서 서명하고, 완료 문서는 보관함에 저장됩니다.', route: '/projecthub' },
      { title: '판매전표 자동 발행', description: '양측 서명이 끝나면 판매전표가 자동으로 기장됩니다(설정 › 회계·세무 › 계정과목·분류에서 프로젝트 매출 계정을 지정한 경우). 세금계산서는 세금·증빙에서 발행합니다.', route: '/tax-invoices' },
      { title: '입금 확인·미수금', description: '입금이 들어오면 장부에 자동 수집됩니다. 프로젝트 화면에서 발행 대비 입금·미수금과 마진을 확인하세요.', route: '/projecthub' },
    ],
  },
  {
    id: 'inventory-start',
    ...
    description: '파는 물건을 등록하고 주문 → 판매·구매로 재고를 움직여, 현황과 이익까지 확인하는 흐름입니다.',
    steps: [
      { title: '품목 등록', description: '재고 → 품목에서 SKU·규격·판매가·매입가를 등록합니다. 모든 재고 흐름의 기초입니다.', route: '/inventory/products' },
      { title: '기초 재고 입력', description: '창고관리에서 현재 갖고 있는 수량을 입·출고와 조정으로 맞춥니다.', route: '/inventory/stock' },
      { title: '주문서 작성', description: '주문에서 주문서·견적을 만듭니다. 주문은 약속이라 아직 재고가 움직이지 않습니다.', route: '/inventory/orders' },
      { title: '판매·구매·생산 처리', description: '주문을 불러와 판매(출고)·구매(입고)·생산(완성)으로 처리하면 그때 재고가 움직입니다.', route: '/inventory/sales' },
      { title: '현황·이익 확인', description: '현황에서 주문·판매·구매·생산 집계를 보고, 이익관리에서 선입선출 원가가 반영된 품목별 이익률을 확인합니다.', route: '/inventory/status' },
    ],
  },
  {
    id: 'team-setup',
