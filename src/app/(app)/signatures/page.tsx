                        ...
                        <tr key={r.id} className={selectedIds.has(r.id) ? "ev-on" : undefined}>
                          ...
                          <td className="signature-table-actions">
                            <div className="signature-request-actions">
                              ...
                              {canRemind && (
                                <button onClick={async () => { if (await appConfirm("이 계약 요청을 취소하시겠습니까?", { danger: true, confirmLabel: "취소 처리" })) cancelMut.mutate(r.id); }} className="btn-icon-danger w-7 h-7 rounded-lg text-sm" aria-label="계약 요청 취소" title="취소(만료 처리)">✕</button>
                              )}
                              {isManager && (
                                <button onClick={async () => { if (await appConfirm("이 계약 요청을 영구 삭제할까요?\n삭제하면 복구할 수 없습니다.", { danger: true })) deleteMut.mutate(r.id); }} disabled={deleteMut.isPending} className="btn-icon-danger w-7 h-7 rounded-lg text-sm" aria-label="영구 삭제" title="영구 삭제"><Ico e="🗑" /></button>
                              )}
                            </div>
                          </td>
                        </tr>
