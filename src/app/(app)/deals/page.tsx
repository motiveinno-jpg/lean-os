  ...
  return (
    <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] overflow-hidden mb-6">
      ...
      <div className="px-5 py-4 flex items-center gap-1">
        {PIPELINE_STAGES.map((ps, idx) => { const stage = stages.find(s => s.stage === ps.key); const status = stage?.status || 'pending'; const isCompleted = status === 'completed'; const isActive = status === 'active'; return (<div key={ps.key} className="flex items-center flex-1"><div className={`flex-1 rounded-xl p-3 text-center transition ${isCompleted ? 'bg-green-500/8 border border-green-500/20' : isActive ? 'bg-blue-500/8 border border-blue-500/20' : 'bg-[var(--bg-surface)] border border-[var(--border)]'}`}><div className="text-lg mb-1">{ps.icon}</div><div className={`text-[10px] font-semibold ${isCompleted ? 'text-green-500' : isActive ? 'text-blue-500' : 'text-[var(--text-dim)]'}`}>{ps.label}</div><div className={`text-[9px] mt-0.5 ${isCompleted ? 'text-green-400' : isActive ? 'text-blue-400' : 'text-[var(--text-dim)]'}`}>{isCompleted ? '완료' : isActive ? '진행중' : '대기'}</div></div>{idx < PIPELINE_STAGES.length - 1 && (<div className={`w-4 h-0.5 mx-0.5 flex-shrink-0 rounded ${isCompleted ? 'bg-green-500/40' : 'bg-[var(--border)]'}`} />)}</div>); })}
      </div>
