...
export default function AuthPage() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    const { data: authData, error: authErr } = await supabase.auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: "https://motiveinno-jpg.github.io/lean-os/auth/verify",
        data: {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] p-8"
          ...
          <div className="flex items-center gap-3 my-6">
            ...
            <span className="text-xs text-[var(--text-dim)] font-medium">또는</span>
            <div className="flex-1 h-px bg-[var(--border)]" />
          </div>
            ...
            <button
              ...
              onClick={async () => {
                await supabase.auth.signInWithOAuth({
                  provider: 'google',
                  options: { redirectTo: `${window.location.origin}/lean-os/auth/verify` }
                });
              }}
