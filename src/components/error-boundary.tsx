...
> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="error-boundary-fallback min-h-screen flex items-center justify-center bg-[var(--bg,#f9fafb)]">
          <div className="error-boundary-card max-w-md w-full mx-4 p-8 rounded-2xl bg-[var(--bg-card)] shadow-lg border border-[var(--border)] text-center">
            ...
            <div className="error-boundary-actions flex gap-3 justify-center">
              <button
                onClick={() => window.location.reload()}
                className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white transition bg-[var(--primary,#6366f1)]"
              >
                새로고침
              </button>
              <button
