  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-[var(--bg-card)] rounded-2xl border border-[var(--border)] p-6 sm:p-8 text-center">
        ...
        <Link
          href="/dashboard"
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[var(--primary)] hover:bg-[var(--primary-hover)] text-white text-sm font-semibold transition active:scale-[0.98] mb-5"
        >
          ← 홈으로 돌아가기
        </Link>

        <div className="border-t border-[var(--border)] pt-4">
          <p className="text-[11px] text-[var(--text-dim)] mb-2">바로 갈 수 있는 곳</p>
          <div className="flex flex-wrap gap-2 justify-center">
            {recs.map((m) => (
              <Link
                key={m.href}
                href={m.href}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-[var(--bg-surface)] hover:bg-[var(--border)] text-[var(--text)] transition"
              >
                <span>{m.emoji}</span>
                {m.label}
              </Link>
