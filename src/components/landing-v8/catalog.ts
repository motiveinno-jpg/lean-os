...
export const CATALOG: Group[] = [
  {
    ...
    menus: [
      ...
      { key: "mypage", name: "마이페이지", href: "/mypage", icon: "user", src: shot("f-mypage-v1"),
        desc: "출퇴근부터 연차·급여명세·증명서까지 내 기록만 모아 봅니다.",
        items: ["오늘 출퇴근·주간 근무", "연차 잔여·휴가 신청", "급여명세·근로계약 확인", "재직·경력증명서 신청"] },
      { key: "copilot", name: "AI 참모", href: "/copilot", icon: "chat", src: shot("f-ai-copilot-v5"),
        desc: "회사 실데이터를 근거로 지금 무엇을 먼저 할지 답합니다.",
        items: ["실데이터 기반 답변", "미수금 회수 우선순위", "화면 위치 안내", "확정은 대표님이"] },
      { key: "support-programs", name: "지원사업추천", href: "/support-programs", icon: "gift", src: null,
  ...
  },
  {
    ...
    menus: [
      { key: "products", name: "품목", href: "/inventory/products", icon: "box", src: shot("f-inv-products-v1"),
        desc: "파는 것을 한 번만 등록하면 재고·판매·구매·생산이 같은 품목을 씁니다.",
        items: ["SKU·규격·단위", "판매가·원가", "안전재고 설정", "수량 안 세는 품목 구분"] },
      { key: "stock", name: "창고관리", href: "/inventory/stock", icon: "layers", src: shot("f-inv-stock-v2"),
        desc: "품목별 현재 수량과 안전재고보다 모자란 품목을 한눈에 봅니다.",
        items: ["창고별 현재고", "안전재고 대비 상태", "움직인 이력 추적", "실사(재고조사)로 차이 조정"] },
      { key: "orders", name: "주문", href: "/inventory/orders", icon: "doc", src: shot("f-inv-orders-v1"),
        desc: "견적과 주문을 한 서식으로 작성합니다. 주문서는 재고를 움직이지 않습니다.",
        items: ["견적·주문 한 서식", "직접 만든 칸 추가", "공급가액·부가세 저장", "판매·구매·생산으로 불러오기"] },
      { key: "sales", name: "판매", href: "/inventory/sales", icon: "trend", src: shot("f-inv-sales-v1"),
        desc: "주문서를 불러와 저장하면 그때 재고가 빠집니다. 나눠서 출고해도 남은 수량을 셉니다.",
        items: ["주문서 불러오기", "부분 출고 관리", "미출고 잔량 추적", "저장 시 재고 자동 반영"] },
      { key: "purchase", name: "구매", href: "/inventory/purchase", icon: "drive", src: shot("f-inv-purchase-v2"),
        desc: "발주한 수량과 실제 입고 수량을 맞춰 보고, 모자란 품목을 찾아 채웁니다.",
        items: ["발주 등록·입고 진행률", "미입고 수량 집계", "모자란 품목 자동 채우기", "저장 시 재고 자동 반영"] },
      { key: "production", name: "생산", href: "/inventory/production", icon: "factory", src: shot("f-inv-production-v1"),
      ...
      { key: "channels", name: "이커머스", href: "/inventory/channels", icon: "link", src: shot("f-inv-channels-v1"),
        desc: "판매채널 주문을 엑셀로 붙여넣어 가져옵니다. 같은 주문은 두 번 들어가지 않습니다.",
        items: ["주문 엑셀 붙여넣기", "중복 주문 자동 차단", "채널 상품코드 ↔ SKU 연결", "가져온 이력 조회"] },
      { key: "inventory-status", name: "현황", href: "/inventory/status", icon: "chart", src: null,
        desc: "주문·판매·구매·생산을 한 화면에서 집계와 그래프로 봅니다.",
        items: ["주문 진행률·납기 지남", "판매·매입 집계(반품 차감)", "자재 부족 예상", "채널별 판매 비중"] },
      { key: "profit", name: "이익관리", href: "/inventory/profit", icon: "trend", src: null,
  ...
  },
  {
    ...
    menus: [
      { key: "schedule", name: "일정 / 할 일", href: "/schedule", icon: "calendar", src: shot("f-schedule-v4"),
        desc: "회사 일정과 내 할 일을 한 화면에서 봅니다.",
        items: ["회사 공유 일정", "나만 보는 개인 일정", "할 일 우선순위", "프로젝트 마감 연동"] },
      { key: "projecthub", name: "프로젝트", href: "/projecthub", icon: "briefcase", src: shot("f-projects-v5"),
        desc: "표·칸반·캘린더·간트 네 가지 보기로 프로젝트를 관리합니다. 진행 단계는 직접 정합니다.",
        items: ["표·칸반·캘린더·간트", "진행 단계 직접 설정", "담당자·마감·첨부", "견적에서 정산까지"] },
      { key: "approvals", name: "결재 허브", href: "/approvals", icon: "check", src: shot("f-approvals-v4"),
        desc: "경비·지출·휴가·구매·연장근무 요청이 하나의 결재함에 모입니다.",
        items: ["내 결재함·내 요청", "다단계 결재선", "일괄 승인·선택 반려", "결재 양식 만들기"] },
      { key: "board", name: "게시판", href: "/board", icon: "board", src: shot("f-board-v4"),
        desc: "공지와 부서 게시글을 같은 시스템 안에서 관리합니다.",
        items: ["전사 공지·고정", "댓글·첨부·투표", "읽음 현황 확인"] },
      { key: "chat", name: "메신저", href: "/chat", icon: "chat", src: shot("f-chat-v4"),
        desc: "프로젝트마다 전용 채널이 열리고, 외부 파트너도 초대할 수 있습니다.",
        items: ["프로젝트·팀·1:1 채널", "파일 공유·멘션", "외부 파트너 초대"] },
      { key: "signatures", name: "전자계약", href: "/signatures", icon: "sign", src: shot("f-contract-v4"),
        desc: "계약서 작성부터 서명·보관까지 한 흐름으로 끝냅니다.",
        items: ["서식·템플릿", "직인 자동 합성", "발송·열람·서명 추적", "열람 전 발송 취소"] },
      { key: "documents", name: "파일보관함", href: "/documents", icon: "folder", src: shot("f-documents-v4"),
        desc: "회사 서류를 폴더로 나눠 권한에 맞게 보관합니다.",
        items: ["폴더·권한 관리", "드래그 업로드", "용량·종류 관리", "통합 검색"] },
      { key: "team", name: "구성원 디렉토리", href: "/team", icon: "users", src: shot("f-team-v1"),
  ...
  },
  {
    ...
    menus: [
      { key: "employees", name: "구성원", href: "/employees", icon: "user", src: shot("f-members-v4"),
        desc: "부서·직급·입사일을 관리하고, 급여는 4대보험·원천세까지 자동으로 계산합니다.",
        items: ["구성원 등록·상세", "4대보험·원천세 자동 계산", "급여 배치·명세서 발송", "메뉴별 권한 부여"] },
      { key: "attendance", name: "근태 관리", href: "/attendance", icon: "clock", src: shot("f-hr-v5"),
        desc: "출퇴근·연차·연장근무가 자동으로 집계됩니다.",
        items: ["원클릭 출퇴근", "연차 발생·사용 이력", "주 52시간 사용률", "여러 달 조회·엑셀"] },
      { key: "hr-templates", name: "근로계약·서식", href: "/hr-templates", icon: "file", src: shot("f-templates-v4"),
  ...
  },
  {
    ...
    menus: [
      ...
      { key: "outlook", name: "자금 전망", href: "/reports/outlook", icon: "clock", src: shot("f-flow-v4"),
        desc: "날짜가 정해진 예정 입출금을 반영해 앞으로의 잔액 곡선을 그립니다.",
        items: ["예정 잔액 곡선", "최저점·부족 시점 경고", "주 단위 자금 달력", "시나리오 조정"] },
      { key: "statements", name: "회계 자료", href: "/reports/statements", icon: "file", src: shot("f-statements-v1"),
  ...
  },
  {
    ...
    menus: [
      { key: "company", name: "회사 기초정보", href: "/settings/company", icon: "sheet", src: shot("f-settings-v1"),
        desc: "사업자 정보와 직인·로고, 회사 공용 양식을 관리합니다.",
        items: ["회사정보·직인·로고", "회사 양식(PDF)"] },
      { key: "people", name: "구성원·초대", href: "/settings/people", icon: "users", src: null,
        desc: "구성원 초대와 합류 요청, 부서를 관리합니다.",
        items: ["구성원 초대", "합류 요청 승인", "부서 관리"] },
      { key: "finance-settings", name: "회계·세무", href: "/settings/finance", icon: "book", src: null,
        desc: "자금 기준·계정과목·회계마감·세무 파트너·4대보험 요율을 정합니다.",
        items: ["자금·통장 기준", "계정과목·분류", "회계마감·기초잔액", "세무 파트너 연결", "4대보험 요율"] },
      { key: "integration", name: "연동·API 키", href: "/settings/integration", icon: "link", src: null,
        desc: "은행·카드 자동 수집을 공동인증서로 연결하고, 외부 서비스 API 키를 등록합니다.",
        items: ["은행연동(공동인증서)", "외부 서비스 API 키"] },
      { key: "system", name: "보안·시스템", href: "/settings/system", icon: "shield", src: null,
        desc: "접속을 허용할 IP를 정합니다.",
        items: ["접속 허용 IP"] },
      { key: "billing", name: "요금제", href: "/billing", icon: "card", src: shot("f-billing-v1"),
  ...
  },
  {
    ...
    menus: [
      { key: "announcements", name: "공지사항", href: "/announcements", icon: "mail", src: shot("f-announcements-v1"),
        desc: "오너뷰의 새 기능과 점검 소식을 앱 안에서 받습니다.",
        items: ["새 기능 소식", "점검 안내", "읽음 표시"] },
      { key: "guide", name: "사용 가이드", href: "/guide", icon: "book", src: shot("f-guide-v1"),
        desc: "메뉴마다 무엇을 하는 곳인지 화면 안에서 찾아봅니다.",
        items: ["메뉴별 사용법", "화면 옆 도움말 서랍", "처음 시작하기"] },
      { key: "support", name: "고객센터", href: "/support", icon: "chat", src: shot("f-support-v1"),
