...
const TEMPLATE_VARS: Record<DocType, string[]> = {
  ...
  //   치환표(lib/quote-pdf.ts 의 vars·values)에 실제로 있는 이름만 권한다. 없는 이름은 경고 없이
  //   빈칸이 되기 때문에, 예전엔 {{대표자}}·{{계약일자}}·{{서명}} 세 개가 완성 PDF 에서 사라졌다 (2026-09-11).
  //   {{서명}} 은 뺐다 — 이 PDF 경로에는 서명 값이 없다(서명은 전자계약이 따로 받는다).
  contract: ["{{거래처명}}", "{{사업자번호}}", "{{대표자명}}", "{{계약일}}", "{{계약금액}}",
             "{{계약시작일}}", "{{계약종료일}}", "{{용역내용}}", "{{결제조건}}"],
