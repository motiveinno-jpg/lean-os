...
//   대표: "애플 페이지처럼 사진 배열·화질·레이아웃이 깔끔하고, 스크롤할 때마다 화면이
 ...
 *     둘째 문장의 시작이 한 줄에 섞여, 어디서 끊어 읽어야 할지 알 수 없다.
 ...
 *        캡처 하네스는 저장소 밖(motive-lean-os-qa) 에 있다.
...
function Fan({ priority = false }: { priority?: boolean }) {
  ...
  /* 자동 롤링 — 펼침이 끝나면 가운데 화면이 저절로 넘어간다(웹·폰 동일, 2026-07-30).
      ...
      {() => (
        <>
          ...
          <div className="lp5-hero-shot">
            {/* ⚠️ 좁은 화면에서 폰 목업으로 바꿔 끼우던 걸 걷어냈다 — 대표: "메인은 웹과
...
//   ⚠️ 대표: "되도록 흩어져 있는 라인 위로는 배치를 안 하는 게 좋을 듯" →
 ...
 *  ⚠️ 창 안의 값과 도착지의 값은 반드시 같아야 한다. 대표: "12개에 있던 데이터가 깔끔하게
...
//    글자가 안 읽힘 — "사라졌다 + 다른 게 나타났다"로 읽혀 폐기했다(웹도 부자연스럽다).
...
function UnifyFlat() {
  ...
  /* ⚠️ 구간에 발만 걸쳐도 재생되면, 사용자가 창까지 스크롤해 내려왔을 때는 이미 다 합쳐져 있다
        ("모바일에서는 이미 합쳐져 있어서, 스크롤 내린 이후 합쳐지는 모습이 보여야 한다").
 ...
 *     폰 화면은 접힌 아래에 있었고, 보려고 스크롤하면 다음 시간대로 넘어가 버렸다.
...
function SceneDay() {
  ...
  //    2.4 였을 때 구간당 266px = 휠 2.66칸이라 3·3·3·2 로 들쭉날쭉했다.
 ...
 *  대표: "스크롤에 따라 화면 넘기기 삭제 (모바일에서 보기가 힘듦).
  ...
  return (
    <div ref={ref}>
      ...
      <div className="lp5-rail-bar">
        {arrows ? (
          // 대표: "자동 재생이 아니라 화살표로 움직이고" — 읽는 속도를 사용자가 정한다
...
//      2.17화면을 먹었다("9개가 길어지다 보니 페이지가 엄청 길어진다").
 ...
 *     알려주는 표시도 없었다(모바일 통일). 조작을 하루·자리에 없어도·AI 와 하나로 맞춘다.
...
//      대표: "애플 인텔리전스처럼 1열로 화면 꽉 차게, 기능이 일하는 모습을 예시로."
  ...
  return (
    <section id="engines" className="lp5-sect lp5-sect-dark">
      ...
      {/* ⚠️ 아래에 "이 밖에도 AI가 알아서 하는 일 3가지" 목록 박스와 "자동화 전체 보기" 버튼이 있었다.
          대표: (1) 그 3가지도 카드로 넣자 → 07~09 카드로 승격,
      ...
      <div className="lp5-wrap">
        <Rise className="lp5-sec-head lp5-sec-head-c">
          ...
          {/* ⚠️ 제목이 "회사 운영, 오직 오너뷰 안에서" 였다 — 히어로·마무리와 '회사 운영'이 겹쳤고,
              대표 원한 건 "이 밖에도 많다"는 이야기였다(그래서 리드가 '이게 다가 아니에요').
        ...
        <div className="lp5-cov4 lp5-swipe">
          {CATALOG.map((g, i) => (
            <Rise key={g.key} delay={i * 60} className="lp5-covg">
              {/* ⚠️ 카드 맨 위에 색 띠를 가로로 깔았었다 — 대표: "박스 윗줄이 디자인적으로 별로".
        ...
        <div className="lp5-proof">
          <Rise className="lp5-case">
            ...
            {/* ⚠️ "만든 회사가 매일 씁니다" 였다 — 페이지에서 유일하게 남은 격식체라 톤이 튀었고,
                대표 직접 "만든 회사도 직접 쓰고 있어요" 로 제안. 해요체 규칙과도 맞는다. */}
          ...
          <Rise delay={90} className="lp5-price">
            ...
            {/* ⚠️ 처음엔 "인원이 늘어도 그대로예요" 였는데 사실이 아니라(기본 5명 포함, 6명째부터
                ...
                대표: "5명이 강조되면 가입률이 떨어진다. 팩트는 좋지만 가입을 망설이게 하는 건
                ...
                ⚠️ "회사 규모" 도 뺐다(2026-07-30) — 규모는 곧 인원이고, 인원은 곧 비용이라
                   읽혀 같은 망설임을 만든다. "일하는 방식에 맞게" 는 대표 좋다고 한 표현이라 남긴다. */}
...
// ══════════════════ 7. 자리에 없어도 — 결재·승인 ══════════════════
//   ⚠️ "회사 밖에서도 PC에서 보던 화면 그대로" 였다. 대표: "위쪽 분위기랑 동떨어진다."
...
function SceneMobile() {
  ...
  //   ⚠️ 자동으로만 넘어가면 지나간 단계를 다시 볼 방법이 없다.
  ...
  /* ⚠️ 폰에서는 목업 한 대와 설명 네 줄이 따로 놀아, 지금 보이는 화면이 어느 설명인지
        맞춰가며 봐야 했다. 한 카드에 설명과 그 화면을 같이 넣고 스와이프로 넘긴다. */
  if (narrow) {
    return (
