  ...
  return (
    <>
      <button ref={btnRef} type="button" id={id} disabled={disabled} onClick={toggle} title={title} style={style}
        className={`date-field-button ${className} inline-flex items-center justify-between gap-2 text-left ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}>
      ...
      {open && typeof document !== "undefined" && createPortal(
        <div id="datefield-pop" style={popStyle} className="date-field-popover rounded-xl border border-[var(--border)] bg-[var(--bg-card)] shadow-2xl p-2.5 select-none">
          <div className="date-field-popover-header flex items-center justify-between mb-2 px-0.5">
          ...
          <div className="date-field-weekday-row grid grid-cols-7 gap-0.5 mb-1">
            {WD.map((w, i) => (
              <div key={w} className={`date-field-weekday h-6 flex items-center justify-center text-[10px] font-semibold ${i === 0 ? "text-red-400" : i === 6 ? "text-blue-400" : "text-[var(--text-dim)]"}`}>{w}</div>
            ))}
          </div>
          <div className="date-field-grid grid grid-cols-7 gap-0.5">
            {cells.map((d, i) => {
              ...
              return (
                <button key={d} type="button" disabled={dis} onClick={() => emit(dayStr)} title={holiday || undefined}
                  className={`date-field-day-btn relative h-7 rounded-lg text-xs font-medium transition flex items-center justify-center
          ...
          {monthHols.length > 0 && (
            <div className="date-field-holidays mt-1.5 px-1 text-[10px] leading-snug text-red-400/90">
              {monthHols.map((h) => `${h.d}일 ${h.name}`).join("  ·  ")}
            </div>
          )}
          <div className="date-field-footer flex items-center justify-between mt-2 pt-2 border-t border-[var(--border)]/60">
            <button type="button" onClick={() => emit("")} className="date-field-clear-btn text-[11px] text-[var(--text-dim)] hover:text-[var(--text)] px-1.5 py-1">지우기</button>
            <button type="button" onClick={() => emit(ymd(today.getFullYear(), today.getMonth() + 1, today.getDate()))} className="date-field-today-btn text-[11px] font-semibold text-[var(--primary)] hover:underline px-1.5 py-1">오늘</button>
          </div>
        </div>,
        document.body,
      )}
