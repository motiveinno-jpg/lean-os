      ...
      {open && (
        <div className="inv-modal" onClick={() => setOpen(false)}>
          <div className="inv-modal-box inv-modal-wide cs-box" onClick={(e) => e.stopPropagation()}>
            ...
            {!snaps.length ? (
              <div className="collect-empty">아직 {year}년에 잠근 달이 없습니다. 회계마감에서 달을 잠그면 {title} 확정본이 남습니다.</div>
            ) : (
              <>
                ...
                {cur && (
                  <>
                    ...
                    {liveCur && (diff.length
                      ? <div className="cs-drift" title="확정본은 잠금을 풀고 다시 잠글 때만 갱신됩니다.">⚠ 잠근 뒤 전표가 바뀌어 <b>{diff.length}개 계정</b>이 지금 숫자와 다릅니다. 전표 현황에서 확인하세요.</div>
