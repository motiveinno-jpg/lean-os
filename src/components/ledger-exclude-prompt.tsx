  ...
  const element = ask ? (
    <div className="approval-detail-modal" onClick={() => done(null)}>
      <div className="pnl-drill exclude-prompt" onClick={(e) => e.stopPropagation()}>
        <div className="pnl-drill-head"><h3 className="text-sm font-bold">장부 제외 <small className="ml-2 font-normal text-[var(--text-dim)]">전표 없이 끝낸 것으로 표시하고 목록에서 뺍니다.</small></h3><button type="button" className="btn-secondary btn-sm" onClick={() => done(null)}>닫기</button></div>
        <div className="pay-form-body space-y-3">
          ...
          <p className="text-[11px] text-[var(--text-dim)]">상태를 장부 제외로 찾으면 언제든 되돌릴 수 있습니다. 전표가 걸린 건은 제외할 수 없습니다.</p>
