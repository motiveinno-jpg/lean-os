  ...
  return (
    <div className="glass-card p-5">
      ...
      <div className="hr-form-upload">
        ...
        <label className={`hr-form-upload-text-btn ${busy ? "bg-[var(--bg-surface)] text-[var(--text-dim)]" : "bg-[var(--primary)] text-white hover:opacity-90"}`} title="PDF를 편집 가능한 텍스트로 변환 — 내용을 고치고 표·서식·{{변수}}를 넣습니다 (권장)">
      ...
      {list.length === 0 ? (
        ...
        <div className="hr-form-list">
          {list.map((t) => (
            <div key={t.id} className="hr-form-row">
        ...
        <div className="hr-form-fill-modal fixed inset-0">
          <div className="bg-[var(--bg-card)] rounded-xl max-w-md w-full max-h-[85vh] overflow-auto p-5" onClick={(e) => e.stopPropagation()}>
            ...
            {fillKeys(filling.tpl).length === 0 ? (
              ...
              <div className="hr-form-fill-fields">
