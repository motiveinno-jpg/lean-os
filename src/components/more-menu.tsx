...
export function MoreMenu({ children, label = "더보기", className = "" }: { children: React.ReactNode; label?: string; className?: string }) {
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button type="button" onClick={() => setOpen((v) => !v)} className="btn-secondary btn-sm whitespace-nowrap" aria-haspopup="menu" aria-expanded={open} title="더 많은 작업">
        ⋯ {label}
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1 z-30 min-w-[168px] py-1 rounded-xl border border-[var(--border)] bg-[var(--bg-card)] shadow-lg" role="menu" onClick={() => setOpen(false)}>
          {children}
        </div>
      )}
    </div>
...
export const MORE_ITEM_CLS = "flex items-center gap-2 w-full text-left px-3 py-2 text-[13px] text-[var(--text)] hover:bg-[var(--bg-surface)] transition no-underline cursor-pointer";
