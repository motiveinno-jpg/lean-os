          ...
          {shown.map((e) => {
            ...
            return (
              <button key={e.id} onClick={() => setSel(e)}
                className="glass-card p-4 text-left hover:-translate-y-0.5 hover:shadow-lg transition group">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {shown.map((e) => {
                const sm = statusMeta(e.status);
                return (
                  <tr key={e.id} onClick={() => setSel(e)} className="border-b border-[var(--border)]/50 hover:bg-[var(--bg-surface)]/50 cursor-pointer">
    ...
    <div className="fixed inset-0 z-[70] flex justify-end" onClick={onClose}>
      ...
      <div className="relative w-full max-w-sm h-full bg-[var(--bg-card)] border-l border-[var(--border)] shadow-2xl flex flex-col overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-5 pb-6 mt-auto space-y-2">
          <Link href="/attendance" className="block w-full text-center px-4 py-2.5 rounded-xl text-xs font-bold text-white transition hover:brightness-110" style={{ background: FLEX.violet }}>
            근태 기록 보기
          </Link>
          {isManager && (
