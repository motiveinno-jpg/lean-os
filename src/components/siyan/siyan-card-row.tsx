...
export function SiyanCardRow({
  ...
  return (
    <div
      onClick={onClick}
      className={`group glass-card p-4 transition-all ${clickable ? "cursor-pointer hover:shadow-md" : ""} ${className}`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {amounts && amounts.length > 0 && (
            <div className={`grid gap-2 mt-3 ${amounts.length === 3 ? "grid-cols-3" : amounts.length === 2 ? "grid-cols-2" : "grid-cols-1"}`}>
              {amounts.map((a, i) => {
                const t = A_TONE[a.tone];
                return (
                  <div key={i} className={`rounded-lg p-2.5 ${t.bg}`}>
