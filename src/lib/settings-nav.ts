// ── 회사 설정 IA 한 벌 ("좌측 사이드바로 메뉴화") ──
//   History: 2026-08-13 대표 좌측 네비를 기각했다. 다만 사유는 '왼쪽'이 아니라 **'왼쪽이 두 개'**
//     — 설정 화면 **안에** 또 패널을 두니 앱 사이드바와 겹쳤다(그래서 상단 밴드 → 8-19 상자 안 탭 한 줄).
//     이번엔 화면 안이 아니라 **앱 사이드바 그 자체**에 펴므로 그 사유에 걸리지 않는다.
//   무엇을 기준으로 펴는가 = **그룹 5개**(leaf 13개가 아니라). 대표 분석 그룹에서 이미 정한
//     "5개까지만 편다. 8개를 다 펴면 사이드바가 길어져 오히려 못 찾는다"를 그대로 따른다.
//   그룹 구성은 2026-08-21 대표 확정판을 그대로 옮긴 것이다(이번에 다시 흔들지 않는다).
//
...
export const SETTINGS_GROUPS: SettingsGroup[] = [
  ...
  {
    //    — '근태·가산수당' 을 인사관리로 내보내고 이 그룹은 '구성원·초대' 하나가 됐다.
  ...
  },
  {
    ...
    leaves: [
      ...
      //   광고 계정은 2026-08-24 **API 키 탭으로 합쳤다** ("광고 계정도 API인데 앞에 API 탭이랑
  ...
  },
  {
    ...
    leaves: [
      //   접속 보안 — 접속 허용 IP. 회사 전체에 걸리는 운영 설정이다.
      //   ※ '결재 상신 총괄 알림'은 2026-08-24 삭제했다(결재는 결재자·참조자에게만 가면 된다.
