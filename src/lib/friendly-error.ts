...
export function reportError(scope: string, err: AnyErr): void {
  ...
  try {
    // Sentry 가 로드돼 있으면 captureException, 없으면 console.error 만.
    const w = window as Window & { Sentry?: { captureException: (e: unknown, ctx?: unknown) => void } };
    if (w.Sentry?.captureException) w.Sentry.captureException(err, { tags: { scope } });
