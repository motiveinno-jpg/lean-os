...
// 표준 전자계약서 본문 — 활성 양식이 없을 때의 기본값(projecthub 계약 작성). 렌더 시 {{회사명}}·{{거래처명}}은
//   자동 치환, 그 외 항목은 [항목] 자리표시로 표시돼 편집기에서 채운다. 제4조는 선금/중도금/잔금 결제일정과 연동.
export const STANDARD_CONTRACT_CONTENT = {
  title: "전자계약서",
  sections: [
    { title: "제1조 (계약 당사자)", content: "갑 (발주자): {{거래처명}} (대표: {{거래처대표}})\n을 (수급자): {{회사명}} (대표: {{대표자명}})" },
    { title: "제2조 (계약의 목적)", content: "본 계약은 갑이 을에게 아래 용역을 위탁하고, 을이 이를 성실히 수행함에 있어 상호 간의 권리와 의무를 정함을 목적으로 한다.\n\n용역 내용: {{용역내용}}" },
    { title: "제3조 (계약 기간)", content: "계약 시작일: {{계약시작일}}\n계약 종료일: {{계약종료일}}\n기간의 연장은 갑과 을의 서면 합의에 의한다." },
    { title: "제4조 (계약 금액 및 지급)", content: "총 계약금액: {{계약금액}}원 (부가가치세 별도)\n지급은 본 계약에 정한 결제 일정(선금·중도금·잔금)에 따르며, 을은 각 회차별로 세금계산서를 발행하고 갑은 세금계산서 수령일로부터 7영업일 이내에 지급한다." },
    { title: "제5조 (산출물 및 권리 귀속)", content: "을이 본 계약에 따라 갑에게 제공한 산출물 및 이와 관련한 지식재산권은 계약금액 완납 시 갑에게 귀속된다. 을은 포트폴리오 목적에 한하여 이를 활용할 수 있다." },
    { title: "제6조 (비밀유지)", content: "갑과 을은 본 계약의 이행 과정에서 알게 된 상대방의 경영상·기술상 비밀정보를 상대방의 사전 서면 동의 없이 제3자에게 누설하지 아니한다. 본 의무는 계약 종료 후 2년간 존속한다." },
    { title: "제7조 (계약의 변경·해지)", content: "본 계약의 변경은 갑과 을의 서면 합의에 의한다. 일방이 본 계약을 중대하게 위반하고 서면에 의한 시정요구를 받은 날로부터 15일 이내에 이를 시정하지 아니한 경우, 상대방은 서면 통지로써 계약을 해지할 수 있다." },
    { title: "제8조 (손해배상)", content: "일방의 귀책사유로 상대방에게 손해가 발생한 경우 귀책 당사자는 그 손해를 배상한다. 다만, 천재지변 등 불가항력에 의한 경우는 그러하지 아니하다." },
    { title: "제9조 (지체상금)", content: "을이 갑이 정한 기일 내에 용역을 완료하지 못한 경우, 지연일수 1일당 지연된 용역 대금의 1000분의 3에 해당하는 금액을 갑에게 지급한다. 다만 그 사유가 갑에게 있거나 불가항력에 의한 경우는 예외로 한다." },
    { title: "제10조 (분쟁 해결 및 관할)", content: "본 계약과 관련하여 분쟁이 발생하는 경우 상호 협의로 해결하되, 협의가 이루어지지 아니한 때에는 갑의 주소지 관할법원을 제1심 전속관할로 한다.\n\n본 계약의 성립을 증명하기 위하여 본 계약을 전자문서로 작성하고 갑과 을이 각각 전자서명한다." },
  ],
};
...
export const DEFAULT_DOC_TEMPLATES = [
  {
    ...
    content_json: STANDARD_CONTRACT_CONTENT,
  },
  {
    name: "마케팅대행 계약서",
    type: "contract",
