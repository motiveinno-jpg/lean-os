...
export async function fetchOutlook(companyId: string, days: number, userId?: string): Promise<OutlookData> {
  ...
  if (!hasBank) gaps.unshift({ key: "nobank", text: "연결된 통장이 없어 현재 잔액이 0입니다", href: "/bank" });
  if (((recur.data || []) as any[]).length + ((fixed.data || []) as any[]).length === 0) gaps.push({ key: "norec", text: "등록된 정기 지출·고정비가 없습니다. 등록 시 전망 정확도가 높아집니다", href: "/payments" });
