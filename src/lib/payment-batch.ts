...
//   소득세 = 국세청 근로소득 간이세액표 원본(@/lib/income-tax). 예전의 12행 근사표는 실제와 어긋나 제거했다.
...
export function calculatePayroll(
  ...
  // 소득세 — 국세청 근로소득 간이세액표 원본. dependents = 공제대상 가족수(본인 포함).
  //   8~20세 자녀 세액공제는 자녀 나이 정보가 없어 0(연말정산에서 정산). 원천징수 100% 기준.
  const it = simplifiedIncomeTax(taxableIncome, dependents, 0);
