...
def part2():
    # ══════════ 08 챕터 Ⅰ ══════════
    chapter("CHAPTER Ⅰ", "지금과 무엇이 다릅니까", "쓰시던 방식과 나란히 놓고 말씀드립니다.", 8)

    # ══════════ 09 비교표 ══════════
    s = plain(9, "쓰시던 방식과 나란히 놓고 말씀드립니다",
              "구식 ERP가 나쁜 것이 아닙니다. 다만 작은 회사에는 무겁습니다.")
    cols = [Inches(0.62), Inches(2.75), Inches(5.55), Inches(8.9)]
    widths = [Inches(2.1), Inches(2.75), Inches(3.3), Inches(3.83)]
    heads = ["", "엑셀 · 수기", "구식 ERP", "오너뷰"]
    top = Inches(2.05)
    ...
    rows = [
        ...
        ("쓰는 말", "사람마다 다름", "회계 용어 위주", "받을 돈 · 낼 돈 · 오늘 챙길 것"),
        ("교육", "불필요하나 실수 잦음", "담당자 교육 필수", "따로 없음 · 화면이 안내"),
        ("모바일", "사실상 불가", "제한적", "웹 그대로"),
        ("확장", "파일이 늘어남", "추가 개발비", "메뉴를 켜고 끄기 · 맞춤은 울트라"),
    ]
    y = top + Inches(0.55)
    for k, a, b, c in rows:
    ...
    # ══════════ 10 도입 전 · 후 ══════════
    s = plain(10, "담당자의 월말이 이렇게 바뀝니다", "경리·총무 담당자가 매달 반복하던 일을 기준으로 적었습니다.")
    ...
    text(s, Inches(0.62), Inches(6.36), Inches(12.11), Inches(0.35),
         [("모으는 일이 사라지면, 그 시간은 ", 12, True, INK), ("판단에 쓰입니다.", 12, True, AMB)], PP_ALIGN.CENTER)

    # ══════════ 11 왜 쉬운가 ══════════
    s = plain(11, "쉬워야 씁니다. 그래서 이렇게 만들었습니다",
              "도입을 미루게 하는 네 가지 이유를 먼저 없앴습니다.")
    for i, (t, d) in enumerate([
            ("설치가 없습니다", "웹에서 바로 씁니다. 서버도, 설치 파일도, 전산 담당자도 필요 없습니다."),
            ("교육이 없습니다", "‘받을 돈·낼 돈·오늘 챙길 것’ 처럼 쓰시던 말로 되어 있습니다."),
            ("연동은 한 번뿐입니다", "공동인증서를 한 번 등록하면 그다음부터는 알아서 들어옵니다."),
            ("안 쓰는 기능은 안 보입니다", "필요한 메뉴만 켜 두면 화면이 단순해집니다. 나중에 켜도 됩니다.")]):
    ...
    # ══════════ 12 챕터 Ⅱ ══════════
    chapter("CHAPTER Ⅱ", "이런 일이 사라집니다", "손이 가던 일곱 가지가 어떻게 사라지는지 화면으로 보여 드립니다.", 12)
    ...
    # ══════════ 20 AI 참모 ══════════
    s = plain(20, "회사 데이터를 아는 AI 에게 물어보십시오",
              "매출·자금·인사 현황을 그냥 물어보시면 됩니다. 답에는 근거 화면이 함께 붙습니다.")
    ...
    for i, (t, d) in enumerate([
            ("무엇이든 물어보세요", "“이번 달 미수금 얼마야?”처럼 평소 말로 물으면 회사 자료를 근거로 답합니다."),
            ("출처를 함께 적습니다", "AI 추천 / 배운 규칙 / 국세청 조회 / 장부 대조 — 어디서 나온 답인지 밝힙니다."),
            ("실행은 사람이", "결재 상신·발송 같은 실행은 사람이 확인한 뒤 누릅니다."),
            ("매일 아침 브리핑", "묻지 않아도 오늘 챙길 것을 다섯 줄로 정리해 둡니다.")]):
