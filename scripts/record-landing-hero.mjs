...
//
//   ⛔ 2026-09-07 — **이 스크립트를 기본 계정(모티브)으로 돌리면 안 된다.**
//      9/4 에 그렇게 찍은 영상의 첫 장면(대시보드)에 **실제 거래처명과 미수금 금액**이 그대로 보였다.
//      랜딩에 나갈 화면은 **가상 인물·가상 거래처뿐인 QA 시드 회사**에서만 찍는다 (결정 220).
//      → 계정을 SHOT_EMAIL/SHOT_PW 로 QA 시드로 바꾸거나, scripts/capture-landing-shots.mjs 처럼 --qa 를 붙인다.
//
