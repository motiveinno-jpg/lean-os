...
${_}
대표이사 ${f} (직인)`}if("contract"===i){let e=(h.paymentSchedule||[]).map(e=>`  - ${e.label} (${e.ratio}%): ₩${e.amount.toLocaleString("ko-KR")} — ${e.condition}`).join("\n");h.body=`표 준 용 역 계 약 서
...
제5조 (계약기간)
${h.contractStartDate||S} ~ ${h.contractEndDate||"프로젝트 완료 시"}
...
</html>`}function p(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}async function m(e){let t=await fetch(e),a=await t.blob();return new Promise((e,t)=>{let n=new FileReader;n.onloadend=()=>e(n.result),n.onerror=t,n.readAsDataURL(a)})}function g(e){let t=Math.abs(e);return`${e<0?"-":""}${t.toLocaleString("ko-KR")}`}function _(e,t){let a=e.getNumberOfPages(),n=e.internal.pageSize.getWidth(),i=e.internal.pageSize.getHeight();for(let s=1;s<=a;s++)e.setPage(s),e.setFontSize(7),(0,r.setKoreanFont)(e,"normal"),e.setTextColor(150,150,150),e.text(`OwnerView Document  |  ${t}  |  Page ${s}/${a}`,n/2,i-8,{align:"center"})}e.s(["generateContractPDF",()=>u,"generateDocumentPDF",()=>l,"generateQuotePDF",()=>d,"issueDocument",()=>c])}]);