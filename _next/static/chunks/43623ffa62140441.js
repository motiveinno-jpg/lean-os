...
${g}
대표이사 ${f} (직인)`}if("contract"===n){let e=(h.paymentSchedule||[]).map(e=>`  - ${e.label} (${e.ratio}%): ₩${e.amount.toLocaleString("ko-KR")} — ${e.condition}`).join("\n");h.body=`표 준 용 역 계 약 서
...
"을" ${c}  대표이사 ______________ (인)`}let{data:I,error:D}=await o.from("documents").insert({company_id:t,deal_id:a,name:b,status:"draft",content_json:h,version:1,created_by:i}).select().single();if(D)throw D;return I.id}async function l(e){let{documentId:a,companyId:n,approverId:r}=e,{data:l}=await t.supabase.from("documents").select("id, name, deal_id, content_json").eq("id",a).single();if(!l||!l.deal_id)return{};let p=l.content_json,m=p?.type;if("invoice"===m||"quote"===m){let e=await d({companyId:n,dealId:l.deal_id,docType:"contract",createdBy:r});await (0,i.dispatchBusinessEvent)({dealId:l.deal_id,eventType:"document_approved",userId:r,referenceId:e,referenceTable:"documents",summary:{title:`계약서 자동 생성됨 (견적서 승인 기반)`}});try{let{data:t}=await o.from("deals").select("name, partners!deals_partner_id_fkey(name, contact_email, contact_phone)").eq("id",l.deal_id).single(),a=t?.partners?.contact_email||"",d=t?.partners?.name||"";try{await (0,s.applyCompanySeal)({documentId:e,companyId:n,appliedBy:r}),await (0,i.dispatchBusinessEvent)({dealId:l.deal_id,eventType:"document_approved",userId:r,referenceId:e,referenceTable:"documents",summary:{title:`계약서 직인 자동 적용`}})}catch{}if(a&&a.includes("@")){let o=await (0,s.createSignatureRequest)({companyId:n,documentId:e,title:`${t?.name||"계약"} - 계약서 서명 요청`,signerName:d,signerEmail:a,createdBy:r});await (0,s.sendSignatureEmail)(o.id),await (0,i.dispatchBusinessEvent)({dealId:l.deal_id,eventType:"document_approved",userId:r,referenceId:o.id,referenceTable:"signature_requests",summary:{title:`서명 요청 자동 발송 (${a})`}})}}catch(e){console.error("Auto-signature request failed:",e)}return{nextAction:"contract_created",createdDocId:e}}if("contract"===m){let e=await c({dealId:l.deal_id,documentId:a,companyId:n,content:p,createdBy:r}),t=await u({dealId:l.deal_id,companyId:n,userId:r,contractTotal:Number(p?.contractTotal||p?.supplyAmount||0),partnerName:p?.partnerName||"",partnerBizNo:p?.partnerBizNo||"",paymentSchedule:p?.paymentSchedule||null,paymentRatio:p?.paymentRatio||null});return{nextAction:"tax_invoice_and_schedule_created",createdInvoiceId:t.invoiceIds?.[0]||void 0,createdDocId:e.pdfDocId}}return{}}async function c(e){let{dealId:t,documentId:a,companyId:n,content:i,createdBy:s}=e;try{let{data:e}=await o.from("companies").select("name, business_number, representative, address, phone").eq("id",n).single(),d=(i?.items||[]).map(e=>({name:e.name||"",spec:e.spec||"",qty:Number(e.quantity||e.qty||1),unitPrice:Number(e.unitPrice||0),amount:Number(e.supplyAmount||e.amount||e.totalAmount||0)})),l=Number(i?.contractTotal||i?.supplyAmount||0),c=(0,r.generateContractPDF)({documentNumber:i?.documentNumber||`CTR-${t.slice(0,8).toUpperCase()}`,date:new Date().toLocaleDateString("ko-KR"),partyA:{name:e?.name||"",representative:e?.representative||"",businessNumber:e?.business_number||"",address:e?.address||"",phone:e?.phone||""},partyB:{name:i?.partnerName||"",representative:i?.counterpartyRepresentative||"",businessNumber:i?.partnerBizNo||"",address:i?.counterpartyAddress||""},contractAmount:l,taxAmount:Math.round(.1*l),totalAmount:Math.round(1.1*l),items:d,contractSubject:i?.dealName||"",contractStartDate:i?.contractStartDate||new Date().toISOString().split("T")[0],contractEndDate:i?.contractEndDate||"",paymentTerms:i?.paymentTerms||"",deliveryDeadline:i?.deliveryDeadline||"",inspectionPeriod:i?.inspectionPeriod||"7영업일",warrantyPeriod:i?.warrantyPeriod||"납품 후 1년",latePenaltyRate:i?.latePenaltyRate||"0.1",specialTerms:i?.specialTerms||""}),{data:u}=await o.from("documents").insert({company_id:n,deal_id:t,name:`${i?.dealName||"계약"} - 계약서 (PDF)`,status:"issued",content_json:{type:"contract_pdf",sourceDocumentId:a,pdfHtml:c,generatedAt:new Date().toISOString(),metadata:{autoGenerated:!0,sourceDocType:"contract"}},content_type:"contract_pdf",version:1,created_by:s}).select("id").single();return{pdfDocId:u?.id}}catch(e){return console.error("Contract PDF generation failed:",e),{}}}async function u(e){let{dealId:t,companyId:n,userId:r,contractTotal:s,partnerName:o,partnerBizNo:d,paymentSchedule:l,paymentRatio:c}=e,u=[];if(s<=0)return{invoiceIds:u};let m=c?.advance??30,g=c?.balance??70,_=l&&Array.isArray(l)&&l.length>0?l:[{label:"선금",ratio:m,amount:Math.round(s*m/100),condition:"계약 후 7일 이내"},{label:"잔금",ratio:g,amount:s-Math.round(s*m/100),condition:"납품 완료 후 14일 이내"}],f=await p({dealId:t,companyId:n,contractTotal:s,userId:r,schedule:_});for(let e=0;e<_.length;e++){let s=_[e];if(s.amount<=0)continue;let l=f[e]||null,c=0===e,p=await (0,a.createTaxInvoice)({companyId:n,dealId:t,type:"sales",counterpartyName:o,counterpartyBizno:d,supplyAmount:s.amount,issueDate:new Date().toISOString().split("T")[0],label:`${s.label} 세금계산서`,revenueScheduleId:l,status:c?"issued":"draft"});p?.id&&(u.push(p.id),await (0,i.dispatchBusinessEvent)({dealId:t,eventType:"invoice_issued",userId:r,referenceId:p.id,referenceTable:"tax_invoices",summary:{amount:Math.round(1.1*s.amount),title:`${s.label} 세금계산서 자동 생성 (${c?"issued":"draft"})`}}))}return{invoiceIds:u}}async function p(e){let{dealId:t,companyId:a,contractTotal:r,userId:s,schedule:d}=e;if(r<=0)return[];let l=new Date,c=[];for(let e=0;e<d.length;e++){let a=d[e],n=new Date(l.getTime()+(0===e?7:60)*864e5),{data:i}=await o.from("deal_revenue_schedule").insert({deal_id:t,label:`${a.label} (${a.ratio}%)`,amount:a.amount,due_date:n.toISOString().split("T")[0],status:"expected",condition_text:a.condition}).select("id").single();i?.id&&c.push(i.id)}let u=d[0];u&&await (0,n.createQueueEntry)({companyId:a,dealId:t,amount:u.amount,description:`${u.label} ${u.ratio}% 입금 예정 (D+7)`,costType:"revenue"});let p=d.map(e=>`${e.label} ${e.amount.toLocaleString()}원`).join(" / ");return await (0,i.dispatchBusinessEvent)({dealId:t,eventType:"milestone_completed",userId:s,referenceId:t,referenceTable:"deals",summary:{title:`매출 스케줄 생성: ${p}`}}),c}async function m(e){let{dealId:t,companyId:n,amount:r,userId:s,revenueScheduleId:d}=e;if(d){await o.from("deal_revenue_schedule").update({status:"received",received_date:new Date().toISOString().split("T")[0]}).eq("id",d);let{data:e}=await o.from("tax_invoices").select("id").eq("revenue_schedule_id",d).single();e&&await (0,a.markInvoiceMatched)(e.id);let{data:n}=await o.from("tax_invoices").select("id, revenue_schedule_id").eq("deal_id",t).eq("status","draft").not("revenue_schedule_id","is",null).order("created_at",{ascending:!0}).limit(1);n&&n.length>0&&await (0,a.issueTaxInvoice)(n[0].id)}let{data:l}=await o.from("deal_revenue_schedule").select("id, amount, status").eq("deal_id",t),c=(l||[]).every(e=>"received"===e.status),u=(l||[]).reduce((e,t)=>e+Number(t.amount||0),0),p=(l||[]).filter(e=>"received"===e.status).reduce((e,t)=>e+Number(t.amount||0),0);c&&p>=u&&(await o.from("deals").update({status:"completed",last_activity_at:new Date().toISOString()}).eq("id",t),await (0,i.dispatchBusinessEvent)({dealId:t,eventType:"deal_status_changed",userId:s,referenceId:t,referenceTable:"deals",summary:{from:"active",to:"completed",reason:"전액 입금 완료"}})),await (0,i.dispatchBusinessEvent)({dealId:t,eventType:"payment_received",userId:s,referenceId:d||t,referenceTable:d?"deal_revenue_schedule":"deals",summary:{amount:r,totalReceived:p,totalExpected:u,progress:u>0?Math.round(p/u*100):0}})}async function g(e){let{data:a}=await t.supabase.from("documents").select("id, name, status, content_json, created_at, updated_at").eq("deal_id",e).order("created_at",{ascending:!0}),{data:n}=await t.supabase.from("tax_invoices").select("id, status, total_amount, issue_date").eq("deal_id",e).neq("status","void"),{data:i}=await o.from("deal_revenue_schedule").select("id, status, amount").eq("deal_id",e),r=[],s=(a||[]).find(e=>{let t=e.content_json;return t?.type==="invoice"||t?.type==="quote"});r.push({stage:"quote",status:s?"approved"===s.status||"locked"===s.status?"completed":"active":"pending",documentId:s?.id,completedAt:s?.status==="approved"?s.updated_at??s.created_at??void 0:void 0});let d=(a||[]).find(e=>{let t=e.content_json;return t?.type==="contract"});r.push({stage:"contract",status:d?"approved"===d.status||"locked"===d.status?"completed":"active":"pending",documentId:d?.id,completedAt:d?.status==="approved"?d.created_at??void 0:void 0}),r.push({stage:"tax_invoice",status:(n||[]).length>0?"completed":"pending",documentId:n?.[0]?.id});let l=(i||[]).length>0;return r.push({stage:"payment_schedule",status:l?"completed":"pending"}),r.push({stage:"payment_received",status:l&&(i||[]).every(e=>"received"===e.status)?"completed":l?"active":"pending"}),r}async function _(e){let{documentId:a,companyId:n,approverId:r,reason:s}=e,{data:o}=await t.supabase.from("documents").select("id, name, status, deal_id, content_json").eq("id",a).single();if(!o)throw Error("문서를 찾을 수 없습니다");let d=o.content_json||{},{error:c}=await t.supabase.from("documents").update({status:"approved",content_json:{...d,forceApproval:{approved:!0,approvedBy:r,approvedAt:new Date().toISOString(),reason:s||"업체 미응답으로 임의 승인"}}}).eq("id",a);if(c)throw c;return(o.deal_id&&await (0,i.dispatchBusinessEvent)({dealId:o.deal_id,eventType:"document_approved",userId:r,referenceId:a,referenceTable:"documents",summary:{title:`${o.name} 임의 승인 (사유: ${s||"업체 미응답"})`}}),o.deal_id)?l({documentId:a,companyId:n,approverId:r}):{nextAction:"force_approved"}}e.s(["createDocumentFromDeal",()=>d,"forceApproveDocument",()=>_,"getDealPipelineStatus",()=>g,"onDocumentApproved",()=>l,"onRevenueReceived",()=>m])},47019,e=>{"use strict";var t=e.i(7471);async function a(e){let{data:a,error:n}=await t.supabase.from("chat_channels").insert({company_id:e.companyId,deal_id:e.dealId||null,sub_deal_id:e.subDealId||null,type:e.type||"deal",name:e.name}).select().single();if(n)throw n;return await t.supabase.from("chat_participants").insert({channel_id:a.id,user_id:e.creatorUserId,role:"OWNER"}),await o(a.id,"channel_created",{created_by:e.creatorUserId}),a}async function n(e){let{data:a,error:n}=await t.supabase.from("chat_messages").insert({channel_id:e.channelId,sender_id:e.senderId,content:e.content,type:e.type||"text",thread_id:e.threadId||null,metadata:e.metadata||null}).select().single();if(n)throw n;return await t.supabase.from("chat_participants").update({last_read_at:new Date().toISOString()}).eq("channel_id",e.channelId).eq("user_id",e.senderId),a}async function i(e,a){let{error:n}=await t.supabase.from("chat_messages").update({pinned:a}).eq("id",e);if(n)throw n}async function r(e){let{data:a,error:n}=await t.supabase.from("chat_participants").insert({channel_id:e.channelId,user_id:e.userId,role:e.role||"member"}).select().single();if(n)throw n;return await o(e.channelId,"user_joined",{user_id:e.userId,role:e.role||"member"}),a}async function s(e,a){let{error:n}=await t.supabase.from("chat_participants").update({last_read_at:new Date().toISOString()}).eq("channel_id",e).eq("user_id",a);if(n)throw n}async function o(e,a,n){await t.supabase.from("chat_events").insert({channel_id:e,event_type:a,data_json:n||null})}async function d(e,t,a){return n({channelId:e,senderId:t,content:a,type:"system"})}async function l(e){if(e.file.size>0xa00000)throw Error("파일 크기는 10MB 이하만 가능합니다.");if(!["image/","application/pdf","application/msword","application/vnd.openxmlformats-","text/csv"].some(t=>e.file.type.startsWith(t)))throw Error("지원하지 않는 파일 형식입니다.");let a=e.file.name.split(".").pop()||"bin",i=`${e.channelId}/${Date.now()}_${Math.random().toString(36).slice(2)}.${a}`,{error:r}=await t.supabase.storage.from("chat-files").upload(i,e.file);if(r)throw r;let{data:s}=t.supabase.storage.from("chat-files").getPublicUrl(i),o=await n({channelId:e.channelId,senderId:e.senderId,content:e.file.name,type:"file",metadata:{file_name:e.file.name,file_url:s.publicUrl,file_size:e.file.size,mime_type:e.file.type}});return await t.supabase.from("chat_files").insert({channel_id:e.channelId,message_id:o.id,file_name:e.file.name,file_url:s.publicUrl,file_size:e.file.size,mime_type:e.file.type}),o}async function c(e){let{data:a,error:n}=await t.supabase.from("chat_messages").insert({channel_id:e.channelId,sender_id:e.senderId,content:e.content,type:e.type||"text",reply_to_id:e.replyToId||null,metadata:e.metadata||null}).select().single();if(n)throw n;if(e.mentionedUserIds.length>0){let n=e.mentionedUserIds.map(t=>({message_id:a.id,channel_id:e.channelId,mentioned_user_id:t}));await t.supabase.from("chat_mentions").insert(n)}return await t.supabase.from("chat_participants").update({last_read_at:new Date().toISOString()}).eq("channel_id",e.channelId).eq("user_id",e.senderId),a}async function u(e,a,n){let{error:i}=await t.supabase.from("chat_reactions").upsert({message_id:e,user_id:a,emoji:n},{onConflict:"message_id,user_id,emoji"});if(i)throw i}async function p(e,a,n){let{error:i}=await t.supabase.from("chat_reactions").delete().eq("message_id",e).eq("user_id",a).eq("emoji",n);if(i)throw i}async function m(e,a){let{error:n}=await t.supabase.from("chat_messages").update({content:a,edited_at:new Date().toISOString()}).eq("id",e);if(n)throw n}async function g(e){let{error:a}=await t.supabase.from("chat_messages").update({deleted_at:new Date().toISOString()}).eq("id",e);if(a)throw a}async function _(e){let a=t.supabase,{data:n,error:i}=await a.from("chat_channels").insert({company_id:e.companyId,name:e.name,description:e.description||null,is_dm:!1}).select().single();if(i)throw i;return n}async function f(e){let a=t.supabase,n=`DM-${Date.now()}`,{data:i,error:r}=await a.from("chat_channels").insert({company_id:e.companyId,name:n,is_dm:!0}).select().single();if(r)throw r;for(let t of e.participantIds)await a.from("chat_members").insert({channel_id:i.id,user_id:t});return i}async function y(e){let a=t.supabase,{data:n}=await a.from("chat_channels").select("invite_token").eq("id",e).single();if(n?.invite_token)return n.invite_token;let i=crypto.randomUUID();return await a.from("chat_channels").update({invite_token:i,allow_guests:!0}).eq("id",e),i}function h(e){let t=window.location.origin;return`${t}/chat?token=${e}`}async function b(e){let{data:a,error:n}=await t.supabase.from("chat_action_cards").insert({message_id:e.messageId,channel_id:e.channelId,card_type:e.cardType,reference_id:e.referenceId,reference_table:e.referenceTable,summary_json:e.summaryJson||{}}).select().single();if(n)throw n;return a}e.s(["addReaction",()=>u,"createActionCard",()=>b,"createChannel",()=>a,"createDMChannel",()=>f,"createTeamChannel",()=>_,"deleteMessage",()=>g,"editMessage",()=>m,"getChatInviteUrl",()=>h,"getOrCreateInviteToken",()=>y,"inviteParticipant",()=>r,"logEvent",()=>o,"markAsRead",()=>s,"removeReaction",()=>p,"sendMessage",()=>n,"sendMessageWithMentions",()=>c,"sendSystemMessage",()=>d,"togglePin",()=>i,"uploadChatFile",()=>l])},88617,e=>{"use strict";var t=e.i(7471),a=e.i(47019);let n={contract_executed:e=>`📝 계약이 체결되었습니다. ${e.title||""}`,payment_received:e=>`💰 입금이 확인되었습니다. ${e.amount?Number(e.amount).toLocaleString()+"원":""}`,milestone_completed:e=>`🎯 마일스톤 완료: ${e.title||e.name||""}`,document_approved:e=>`✅ 문서가 승인되었습니다: ${e.title||""}`,document_locked:e=>`🔒 문서가 잠금되었습니다: ${e.title||""}`,deal_status_changed:e=>`📊 딜 상태 변경: ${e.from||""} → ${e.to||""}`,cost_approved:e=>`💳 비용이 승인되었습니다. ${e.amount?Number(e.amount).toLocaleString()+"원":""}`,invoice_issued:e=>`📋 세금계산서가 발행되었습니다. ${e.amount?Number(e.amount).toLocaleString()+"원":""}`,quote_approved:e=>`📄 견적서가 승인되었습니다 → 계약서 자동 생성. ${e.title||""}`,contract_signed:e=>`✍️ 계약서가 서명되었습니다. ${e.title||""}`,payment_schedule_created:e=>`📅 매출 스케줄이 생성되었습니다. 선금 ${e.advance?Number(e.advance).toLocaleString()+"원":""} / 잔금 ${e.balance?Number(e.balance).toLocaleString()+"원":""}`,revenue_received:e=>`💰 매출 입금: ${e.amount?Number(e.amount).toLocaleString()+"원":""} (${e.progress||0}%)`},i={contract_executed:"document",payment_received:"payment",milestone_completed:"milestone",document_approved:"approval",document_locked:"document",deal_status_changed:"quote",cost_approved:"payment",invoice_issued:"document",quote_approved:"approval",contract_signed:"document",payment_schedule_created:"payment",revenue_received:"payment"};async function r(e){let{dealId:r,eventType:s,userId:o,referenceId:d,referenceTable:l,summary:c={}}=e,{data:u}=await t.supabase.from("chat_channels").select("id").eq("deal_id",r).eq("is_archived",!1).limit(1).maybeSingle();if(!u)return;let p=n[s],m=p?p(c):`시스템 이벤트: ${s}`,g=await (0,a.sendSystemMessage)(u.id,o,m),_=i[s]||"document";await (0,a.createActionCard)({messageId:g.id,channelId:u.id,cardType:_,referenceId:d,referenceTable:l,summaryJson:{...c,eventType:s}}),await (0,a.logEvent)(u.id,s,{user_id:o,reference_id:d,reference_table:l,...c})}e.s(["dispatchBusinessEvent",()=>r])},87200,e=>{"use strict";var t=e.i(55749),a=e.i(45700),n=e.i(7471),i=e.i(40352),r=e.i(94286);let s=n.supabase;async function o(e,t="DOC"){let a=new Date,n=`${a.getFullYear()}${String(a.getMonth()+1).padStart(2,"0")}`,i=`${t}-${n}-%`,{data:r}=await s.from("documents").select("document_number").eq("company_id",e).like("document_number",i).order("document_number",{ascending:!1}).limit(1),d=1;if(r&&r.length>0&&r[0].document_number){let e=r[0].document_number.split("-"),t=parseInt(e[e.length-1],10);isNaN(t)||(d=t+1)}return`${t}-${n}-${String(d).padStart(4,"0")}`}async function d(e){let n=new t.default("p","mm","a4");await (0,r.loadKoreanFont)(n);let i=n.internal.pageSize.getWidth(),s=n.internal.pageSize.getHeight(),o=20;n.setFontSize(12),(0,r.setKoreanFont)(n,"normal"),n.setTextColor(100,100,100),n.text(e.companyName,14,o),e.documentNumber&&(n.setFontSize(9),n.text(e.documentNumber,i-14,o,{align:"right"})),o+=12,n.setFontSize(18),(0,r.setKoreanFont)(n,"bold"),n.setTextColor(30,30,30),n.text(e.title,i/2,o,{align:"center"}),o+=14,n.setDrawColor(200,200,200),n.line(14,o,i-14,o),o+=10;let d=e.content.split("\n").map(e=>[e]);(0,a.default)(n,{startY:o,body:d,theme:"plain",styles:{fontSize:10,cellPadding:{top:1.5,bottom:1.5,left:2,right:2},textColor:[40,40,40],lineWidth:0,font:"NanumGothic"},columnStyles:{0:{cellWidth:i-28}},margin:{left:14,right:14},tableLineColor:[255,255,255],tableLineWidth:0}),o=n.lastAutoTable.finalY+15;let l=e.issueDate||new Date().toLocaleDateString("ko-KR");o>s-80&&(n.addPage(),o=20),n.setFontSize(10),(0,r.setKoreanFont)(n,"normal"),n.setTextColor(60,60,60),n.text(l,i/2,o,{align:"center"}),o+=10;let c=e.companyInfo;if(c){let t=[];for(let a of(t.push(e.companyName),c.address&&t.push(c.address),c.businessNumber&&t.push(`사업자등록번호: ${c.businessNumber}`),c.phone&&t.push(`TEL: ${c.phone}`),c.representative&&t.push(`대표이사: ${c.representative}`),n.setFontSize(9),t))n.text(a,i/2,o,{align:"center"}),o+=5}if(e.applyStamp&&e.sealUrl)try{let t=await m(e.sealUrl),a=o-10;n.addImage(t,"PNG",i-14-30,a,30,30)}catch{console.warn("Seal image load failed, skipping stamp overlay")}return _(n,e.companyName),n.output("blob")}async function l(e){let n=new t.default("p","mm","a4");await (0,r.loadKoreanFont)(n);let i=n.internal.pageSize.getWidth(),s=15;n.setFontSize(20),(0,r.setKoreanFont)(n,"bold"),n.setTextColor(30,30,30),n.text("견 적 서",i/2,s,{align:"center"}),s+=12,n.setFontSize(9),(0,r.setKoreanFont)(n,"normal"),n.setTextColor(80,80,80),n.text(`No. ${e.documentNumber}`,14,s),n.text(`Date: ${new Date().toLocaleDateString("ko-KR")}`,i-14,s,{align:"right"}),s+=8;let o=[["수 신",`${e.counterparty} 귀하`],["발 신",e.companyInfo.name],["대표이사",e.companyInfo.representative||"-"],["사업자번호",e.companyInfo.businessNumber||"-"],["주 소",e.companyInfo.address||"-"],["연락처",e.companyInfo.phone||"-"]];e.managerName&&o.push(["담 당 자",e.managerName+(e.managerContact?` (${e.managerContact})`:"")]),(0,a.default)(n,{startY:s,body:o,theme:"grid",styles:{fontSize:9,cellPadding:3,font:"NanumGothic"},columnStyles:{0:{cellWidth:30,fontStyle:"bold",fillColor:[245,247,250]},1:{cellWidth:i-58}},margin:{left:14,right:14}}),s=n.lastAutoTable.finalY+6,n.setFillColor(59,130,246),n.roundedRect(14,s,i-28,12,2,2,"F"),n.setFontSize(12),(0,r.setKoreanFont)(n,"bold"),n.setTextColor(255,255,255),n.text(`합계금액:  ${g(e.totalAmount)} 원 (VAT 포함)`,i/2,s+8,{align:"center"}),s+=18;let d=e.items.map((e,t)=>[String(t+1),e.name,e.spec||"-",e.qty.toLocaleString("ko-KR"),g(e.unitPrice),g(e.amount)]);(0,a.default)(n,{startY:s,head:[["No","품 명","규 격","수 량","단 가","금 액"]],body:d,theme:"grid",styles:{fontSize:8,cellPadding:3,halign:"center",font:"NanumGothic"},headStyles:{fillColor:[59,130,246],textColor:255,fontStyle:"bold",font:"NanumGothic"},columnStyles:{0:{cellWidth:12},1:{cellWidth:50,halign:"left"},2:{cellWidth:30},3:{cellWidth:20},4:{cellWidth:30,halign:"right"},5:{cellWidth:35,halign:"right"}},margin:{left:14,right:14},alternateRowStyles:{fillColor:[248,249,250]}}),s=n.lastAutoTable.finalY+2,(0,a.default)(n,{startY:s,body:[["공급가액",`${g(e.supplyAmount)} 원`],["부가세 (10%)",`${g(e.taxAmount)} 원`],["합계금액",`${g(e.totalAmount)} 원`]],theme:"grid",styles:{fontSize:9,cellPadding:3,font:"NanumGothic"},columnStyles:{0:{cellWidth:40,fontStyle:"bold",fillColor:[245,247,250],halign:"center"},1:{halign:"right"}},margin:{left:i-14-100,right:14}}),s=n.lastAutoTable.finalY+6;let l=[];if(e.validUntil&&l.push(["유효기간",e.validUntil]),e.deliveryDate&&l.push(["납품일",e.deliveryDate]),e.bankInfo){let t=e.bankInfo.accountHolder?` (${e.bankInfo.accountHolder})`:"";l.push(["입금계좌",`${e.bankInfo.bankName} ${e.bankInfo.accountNumber}${t}`])}if(e.notes&&l.push(["비 고",e.notes]),l.length>0&&((0,a.default)(n,{startY:s,body:l,theme:"grid",styles:{fontSize:9,cellPadding:3,font:"NanumGothic"},columnStyles:{0:{cellWidth:30,fontStyle:"bold",fillColor:[245,247,250]}},margin:{left:14,right:14}}),s=n.lastAutoTable.finalY+6),e.sealUrl)try{let t=await m(e.sealUrl);n.addImage(t,"PNG",i-14-30-5,s,30,30)}catch{console.warn("Seal image load failed, skipping stamp overlay")}return _(n,e.companyInfo.name),n.output("blob")}async function c(e,t,a){let n=await o(a),r=new Date().toISOString(),{error:d}=await s.from("documents").update({document_number:n,status:"issued",issued_at:r,locked_at:r}).eq("id",e);if(d)throw d;await (0,i.logAudit)({companyId:a,userId:t,entityType:"document",entityId:e,action:"issue",afterJson:{document_number:n,status:"issued",issued_at:r,locked_at:r}})}function u(e){let{documentNumber:t,date:a,partyA:n,partyB:i,contractAmount:r,taxAmount:s,totalAmount:o,items:d,contractSubject:l,contractStartDate:c,contractEndDate:u,paymentTerms:m,deliveryDeadline:g,inspectionPeriod:_,warrantyPeriod:f,latePenaltyRate:y,specialTerms:h,sealUrlA:b,sealUrlB:v}=e,w=d.length>0?d.map((e,t)=>`
        ...
        </tr>`).join("\n"):`<tr><td colspan="6" style="text-align:center;color:#999;">품목 없음</td></tr>`,x=b?`<img src="${p(b)}" alt="갑 직인" style="width:60px;height:60px;margin-left:8px;vertical-align:middle;" />`:'<span style="display:inline-block;width:60px;height:60px;border:1px solid #ccc;border-radius:50%;text-align:center;line-height:60px;color:#ccc;font-size:11px;margin-left:8px;vertical-align:middle;">인</span>',$=v?`<img src="${p(v)}" alt="을 직인" style="width:60px;height:60px;margin-left:8px;vertical-align:middle;" />`:'<span style="display:inline-block;width:60px;height:60px;border:1px solid #ccc;border-radius:50%;text-align:center;line-height:60px;color:#ccc;font-size:11px;margin-left:8px;vertical-align:middle;">인</span>',S=h?p(h).replace(/\n/g,"<br/>"):"해당 없음";return`<!DOCTYPE html>
<html lang="ko">
<head>
<meta charset="UTF-8" />
