...
발행일: ${n.issue_date}`})}catch(e){console.error("autoCreateExpenseReport failed:",e)}}async function i(e){let a=.01;try{let{data:n}=await t.supabase.from("companies").select("tax_settings").eq("id",e).single(),i=n?.tax_settings;i?.matching_tolerance!=null&&i.matching_tolerance>=0&&i.matching_tolerance<=100&&(a=i.matching_tolerance/100)}catch{}let{data:n}=await t.supabase.from("tax_invoices").select("*, deals(*)").eq("company_id",e).eq("type","sales").neq("status","void");if(!n)return[];let{data:i}=await t.supabase.from("deal_revenue_schedule").select("*, deals!inner(company_id)").eq("deals.company_id",e).eq("status","received"),r=new Map;return(i||[]).forEach(e=>{let t=e.deal_id;r.set(t,(r.get(t)||0)+Number(e.amount||0))}),n.map(e=>{let t=e.deals,n=Number(t?.contract_total||0),i=Number(e.supply_amount||0),o=Number(e.tax_amount||0),s=Number(e.total_amount||0),l=r.get(e.deal_id)||0,d=n>0&&Math.abs(n-i)/n<=a,c=s>0&&Math.abs(s-l)/s<=a;return{invoiceId:e.id,dealId:e.deal_id,dealName:t?.name||null,invoiceAmount:s,invoiceSupplyAmount:i,invoiceTaxAmount:o,contractAmount:n,receivedAmount:l,amountMatch:d,paymentMatch:c,fullMatch:d&&c,gap:s-l}})}async function r(e){let{error:a}=await t.supabase.from("tax_invoices").update({status:"matched"}).eq("id",e);if(a)throw a}async function o(e){let{data:a,error:n}=await t.supabase.from("tax_invoices").update({status:"issued",issue_date:new Date().toISOString().split("T")[0]}).eq("id",e).eq("status","draft").select().single();if(n)throw n;return a}async function s(e,a,n="monthly"){let i=`${a}-01-01`,r=`${a}-12-31`,{data:o}=await t.supabase.from("tax_invoices").select("type, supply_amount, tax_amount, total_amount, issue_date").eq("company_id",e).neq("status","void").gte("issue_date",i).lte("issue_date",r);if(!o||0===o.length)return[];let l=new Map;return o.forEach(e=>{let t=(e=>{let t=new Date(e).getMonth()+1;if("annual"===n)return`${a}`;if("quarterly"===n){let e=Math.ceil(t/3);return`${a}-Q${e}`}return`${a}-${String(t).padStart(2,"0")}`})(e.issue_date);l.has(t)||l.set(t,{period:t,salesCount:0,purchaseCount:0,salesSupply:0,salesTax:0,salesTotal:0,purchaseSupply:0,purchaseTax:0,purchaseTotal:0,vatPayable:0});let i=l.get(t),r=Number(e.supply_amount||0),o=Number(e.tax_amount||0),s=Number(e.total_amount||0);"sales"===e.type?(i.salesCount++,i.salesSupply+=r,i.salesTax+=o,i.salesTotal+=s):(i.purchaseCount++,i.purchaseSupply+=r,i.purchaseTax+=o,i.purchaseTotal+=s)}),l.forEach(e=>{e.vatPayable=e.salesTax-e.purchaseTax}),Array.from(l.values()).sort((e,t)=>e.period.localeCompare(t.period))}async function l(e,a){let n=t.supabase,i=await s(e,a,"quarterly"),{data:r}=await n.from("card_deduction_summary").select("*").eq("company_id",e),o=new Map;(r||[]).forEach(e=>{let t=new Date(e.month);if(t.getFullYear()!==a)return;let n=Math.ceil((t.getMonth()+1)/3),i=`${a}-Q${n}`;o.set(i,(o.get(i)||0)+Number(e.estimated_vat_deduction||0))});let l={[`${a}-Q1`]:`${a}-04-25`,[`${a}-Q2`]:`${a}-07-25`,[`${a}-Q3`]:`${a}-10-25`,[`${a}-Q4`]:`${a+1}-01-25`};return[`${a}-Q1`,`${a}-Q2`,`${a}-Q3`,`${a}-Q4`].map(e=>{let t=i.find(t=>t.period===e),a=t?.salesTax||0,n=t?.purchaseTax||0,r=o.get(e)||0;return{quarter:e,salesTax:a,purchaseTax:n,cardDeduction:r,netVAT:a-n-r,dueDate:l[e]||""}})}function d(e){return e.map(e=>({type:"매출"===e["구분"]||"발행"===e["유형"]?"sales":"purchase",counterpartyName:String(e["거래처명"]||e["상호"]||""),counterpartyBizno:String(e["사업자번호"]||e["사업자등록번호"]||""),supplyAmount:Number(e["공급가액"]||e["공급가"]||0),taxAmount:Number(e["세액"]||e["부가세"]||0),totalAmount:Number(e["합계금액"]||e["합계"]||0),issueDate:String(e["발행일"]||e["작성일자"]||"")})).filter(e=>e.counterpartyName&&e.supplyAmount>0)}async function c(e){let{data:{session:a}}=await t.supabase.auth.getSession();if(!a)throw Error("로그인이 필요합니다");let n=await t.supabase.functions.invoke("sync-hometax-invoices",{body:{type:e.type||"both",start_date:e.startDate,end_date:e.endDate}});if(n.error)throw n.error;return n.data}async function u(e){let{data:{session:a}}=await t.supabase.auth.getSession();if(!a)throw Error("로그인이 필요합니다");let n=await t.supabase.functions.invoke("modify-tax-invoice",{body:{invoice_id:e.invoiceId,reason:e.reason,new_supply_amount:e.newSupplyAmount,modification_date:e.modificationDate}});if(n.error)throw n.error;return n.data}async function p(e){let a=t.supabase,{data:n,error:i}=await a.from("tax_invoice_queue").select("*, deals(name)").eq("company_id",e).in("status",["pending","needs_approval","processing"]).order("created_at",{ascending:!1});if(i)throw i;return n||[]}async function m(e,a){let n=t.supabase,{error:i}=await n.from("tax_invoice_queue").update({status:"pending",approved_by:a,approved_at:new Date().toISOString()}).eq("id",e);if(i)throw i}async function y(e,a=20){let n=t.supabase,{data:i,error:r}=await n.from("hometax_sync_log").select("*").eq("company_id",e).order("created_at",{ascending:!1}).limit(a);if(r)throw r;return i||[]}async function f(e,a){let n=a.map(t=>({company_id:e,type:t.type,counterparty_name:t.counterpartyName,counterparty_bizno:t.counterpartyBizno||null,supply_amount:t.supplyAmount,tax_amount:t.taxAmount||Math.round(.1*t.supplyAmount),total_amount:t.totalAmount||Math.round(1.1*t.supplyAmount),issue_date:t.issueDate,status:"sales"===t.type?"issued":"received"})),{data:i,error:r}=await t.supabase.from("tax_invoices").insert(n).select();if(r)throw r;return i}e.s(["INVOICE_STATUS",0,{draft:{label:"작성중",bg:"bg-gray-500/10",text:"text-gray-400"},issued:{label:"발행",bg:"bg-blue-500/10",text:"text-blue-400"},received:{label:"수취",bg:"bg-blue-500/10",text:"text-blue-400"},matched:{label:"매칭완료",bg:"bg-green-500/10",text:"text-green-400"},modified:{label:"수정발행",bg:"bg-orange-500/10",text:"text-orange-400"},void:{label:"무효",bg:"bg-red-500/10",text:"text-red-400"}},"INVOICE_TYPES",0,[{value:"sales",label:"매출 (발행)"},{value:"purchase",label:"매입 (수취)"}],"approveQueueItem",()=>m,"bulkImportTaxInvoices",()=>f,"createTaxInvoice",()=>a,"getHomeTaxSyncLogs",()=>y,"getInvoiceQueue",()=>p,"getTaxInvoiceSummary",()=>s,"getVATPreview",()=>l,"issueTaxInvoice",()=>o,"markInvoiceMatched",()=>r,"modifyTaxInvoice",()=>u,"parseHomeTaxExcel",()=>d,"syncHomeTaxInvoices",()=>c,"threeWayMatch",()=>i])},10160,e=>{"use strict";function t(e){return(t="function"==typeof Symbol&&"symbol"==typeof Symbol.iterator?function(e){return typeof e}:function(e){return e&&"function"==typeof Symbol&&e.constructor===Symbol&&e!==Symbol.prototype?"symbol":typeof e})(e)}e.s(["default",()=>t])},94286,e=>{"use strict";let t=null;async function a(e){if(!t)try{let e=await fetch("https://fonts.gstatic.com/s/nanumgothic/v23/PN_3Rfi-oW3hYwmKDpxS7F_z_tLfxno73g.ttf");if(!e.ok)throw Error(`Font fetch failed: ${e.status}`);let a=await e.arrayBuffer(),n=new Uint8Array(a),i="";for(let e=0;e<n.length;e+=8192){let t=n.subarray(e,Math.min(e+8192,n.length));i+=String.fromCharCode.apply(null,t)}t=btoa(i)}catch(e){console.warn("Korean font load failed, falling back to helvetica:",e);return}e.addFileToVFS("NanumGothic-Regular.ttf",t),e.addFont("NanumGothic-Regular.ttf","NanumGothic","normal"),e.addFont("NanumGothic-Regular.ttf","NanumGothic","bold")}function n(e,t="normal"){try{e.setFont("NanumGothic",t)}catch{e.setFont("helvetica",t)}}e.s(["loadKoreanFont",()=>a,"setKoreanFont",()=>n])},87200,e=>{"use strict";var t=e.i(55749),a=e.i(45700),n=e.i(7471),i=e.i(40352),r=e.i(94286);let o=n.supabase;async function s(e,t="DOC"){let a=new Date,n=`${a.getFullYear()}${String(a.getMonth()+1).padStart(2,"0")}`,i=`${t}-${n}-%`,{data:r}=await o.from("documents").select("document_number").eq("company_id",e).like("document_number",i).order("document_number",{ascending:!1}).limit(1),l=1;if(r&&r.length>0&&r[0].document_number){let e=r[0].document_number.split("-"),t=parseInt(e[e.length-1],10);isNaN(t)||(l=t+1)}return`${t}-${n}-${String(l).padStart(4,"0")}`}async function l(e){let n=new t.default("p","mm","a4");await (0,r.loadKoreanFont)(n);let i=n.internal.pageSize.getWidth(),o=n.internal.pageSize.getHeight(),s=20;n.setFontSize(12),(0,r.setKoreanFont)(n,"normal"),n.setTextColor(100,100,100),n.text(e.companyName,14,s),e.documentNumber&&(n.setFontSize(9),n.text(e.documentNumber,i-14,s,{align:"right"})),s+=12,n.setFontSize(18),(0,r.setKoreanFont)(n,"bold"),n.setTextColor(30,30,30),n.text(e.title,i/2,s,{align:"center"}),s+=14,n.setDrawColor(200,200,200),n.line(14,s,i-14,s),s+=10;let l=e.content.split("\n").map(e=>[e]);(0,a.default)(n,{startY:s,body:l,theme:"plain",styles:{fontSize:10,cellPadding:{top:1.5,bottom:1.5,left:2,right:2},textColor:[40,40,40],lineWidth:0,font:"NanumGothic"},columnStyles:{0:{cellWidth:i-28}},margin:{left:14,right:14},tableLineColor:[255,255,255],tableLineWidth:0}),s=n.lastAutoTable.finalY+15;let d=e.issueDate||new Date().toLocaleDateString("ko-KR");s>o-80&&(n.addPage(),s=20),n.setFontSize(10),(0,r.setKoreanFont)(n,"normal"),n.setTextColor(60,60,60),n.text(d,i/2,s,{align:"center"}),s+=10;let c=e.companyInfo;if(c){let t=[];for(let a of(t.push(e.companyName),c.address&&t.push(c.address),c.businessNumber&&t.push(`사업자등록번호: ${c.businessNumber}`),c.phone&&t.push(`TEL: ${c.phone}`),c.representative&&t.push(`대표이사: ${c.representative}`),n.setFontSize(9),t))n.text(a,i/2,s,{align:"center"}),s+=5}if(e.applyStamp&&e.sealUrl)try{let t=await m(e.sealUrl),a=s-10;n.addImage(t,"PNG",i-14-30,a,30,30)}catch{console.warn("Seal image load failed, skipping stamp overlay")}return f(n,e.companyName),n.output("blob")}async function d(e){let n=new t.default("p","mm","a4");await (0,r.loadKoreanFont)(n);let i=n.internal.pageSize.getWidth(),o=15;n.setFontSize(20),(0,r.setKoreanFont)(n,"bold"),n.setTextColor(30,30,30),n.text("견 적 서",i/2,o,{align:"center"}),o+=12,n.setFontSize(9),(0,r.setKoreanFont)(n,"normal"),n.setTextColor(80,80,80),n.text(`No. ${e.documentNumber}`,14,o),n.text(`Date: ${new Date().toLocaleDateString("ko-KR")}`,i-14,o,{align:"right"}),o+=8;let s=[["수 신",`${e.counterparty} 귀하`],["발 신",e.companyInfo.name],["대표이사",e.companyInfo.representative||"-"],["사업자번호",e.companyInfo.businessNumber||"-"],["주 소",e.companyInfo.address||"-"],["연락처",e.companyInfo.phone||"-"]];e.managerName&&s.push(["담 당 자",e.managerName+(e.managerContact?` (${e.managerContact})`:"")]),(0,a.default)(n,{startY:o,body:s,theme:"grid",styles:{fontSize:9,cellPadding:3,font:"NanumGothic"},columnStyles:{0:{cellWidth:30,fontStyle:"bold",fillColor:[245,247,250]},1:{cellWidth:i-58}},margin:{left:14,right:14}}),o=n.lastAutoTable.finalY+6,n.setFillColor(59,130,246),n.roundedRect(14,o,i-28,12,2,2,"F"),n.setFontSize(12),(0,r.setKoreanFont)(n,"bold"),n.setTextColor(255,255,255),n.text(`합계금액:  ${y(e.totalAmount)} 원 (VAT 포함)`,i/2,o+8,{align:"center"}),o+=18;let l=e.items.map((e,t)=>[String(t+1),e.name,e.spec||"-",e.qty.toLocaleString("ko-KR"),y(e.unitPrice),y(e.amount)]);(0,a.default)(n,{startY:o,head:[["No","품 명","규 격","수 량","단 가","금 액"]],body:l,theme:"grid",styles:{fontSize:8,cellPadding:3,halign:"center",font:"NanumGothic"},headStyles:{fillColor:[59,130,246],textColor:255,fontStyle:"bold",font:"NanumGothic"},columnStyles:{0:{cellWidth:12},1:{cellWidth:50,halign:"left"},2:{cellWidth:30},3:{cellWidth:20},4:{cellWidth:30,halign:"right"},5:{cellWidth:35,halign:"right"}},margin:{left:14,right:14},alternateRowStyles:{fillColor:[248,249,250]}}),o=n.lastAutoTable.finalY+2,(0,a.default)(n,{startY:o,body:[["공급가액",`${y(e.supplyAmount)} 원`],["부가세 (10%)",`${y(e.taxAmount)} 원`],["합계금액",`${y(e.totalAmount)} 원`]],theme:"grid",styles:{fontSize:9,cellPadding:3,font:"NanumGothic"},columnStyles:{0:{cellWidth:40,fontStyle:"bold",fillColor:[245,247,250],halign:"center"},1:{halign:"right"}},margin:{left:i-14-100,right:14}}),o=n.lastAutoTable.finalY+6;let d=[];if(e.validUntil&&d.push(["유효기간",e.validUntil]),e.deliveryDate&&d.push(["납품일",e.deliveryDate]),e.bankInfo){let t=e.bankInfo.accountHolder?` (${e.bankInfo.accountHolder})`:"";d.push(["입금계좌",`${e.bankInfo.bankName} ${e.bankInfo.accountNumber}${t}`])}if(e.notes&&d.push(["비 고",e.notes]),d.length>0&&((0,a.default)(n,{startY:o,body:d,theme:"grid",styles:{fontSize:9,cellPadding:3,font:"NanumGothic"},columnStyles:{0:{cellWidth:30,fontStyle:"bold",fillColor:[245,247,250]}},margin:{left:14,right:14}}),o=n.lastAutoTable.finalY+6),e.sealUrl)try{let t=await m(e.sealUrl);n.addImage(t,"PNG",i-14-30-5,o,30,30)}catch{console.warn("Seal image load failed, skipping stamp overlay")}return f(n,e.companyInfo.name),n.output("blob")}async function c(e,t,a){let n=await s(a),r=new Date().toISOString(),{error:l}=await o.from("documents").update({document_number:n,status:"issued",issued_at:r,locked_at:r}).eq("id",e);if(l)throw l;await (0,i.logAudit)({companyId:a,userId:t,entityType:"document",entityId:e,action:"issue",afterJson:{document_number:n,status:"issued",issued_at:r,locked_at:r}})}function u(e){let{documentNumber:t,date:a,partyA:n,partyB:i,contractAmount:r,taxAmount:o,totalAmount:s,items:l,contractSubject:d,contractStartDate:c,contractEndDate:u,paymentTerms:m,deliveryDeadline:y,inspectionPeriod:f,warrantyPeriod:_,latePenaltyRate:g,specialTerms:b,sealUrlA:h,sealUrlB:v}=e,w=l.length>0?l.map((e,t)=>`
        <tr>
...
<div class="contract-page">
  ...
  <div class="article">
    ...
    <div class="article-body">
      <p>① 을은 ${p(y||"별도 협의")}까지 본 건의 결과물(이하 "납품물"이라 한다)을 갑에게 납품\xb7인도한다.</p>
  ...
  <div class="article">
    ...
    <div class="article-body">
      <p>① 갑은 납품일로부터 ${p(f)} 이내에 납품물의 수량\xb7품질\xb7규격 등을 검수하여야 한다.</p>
  ...
  <div class="article">
    ...
    <div class="article-body">
      <p>① 을은 납품물에 대하여 검수 완료일로부터 ${p(_)} 동안 하자보수 책임을 진다.</p>
  ...
  <div class="article">
    ...
    <div class="article-body">
      <p>① 을이 납품기한을 초과하여 이행하는 경우 지체일수 1일당 계약금액의 ${p(g)}%에 해당하는 금액을 지체상금으로 갑에게 납부하여야 한다.</p>
...
</html>`}function p(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}async function m(e){let t=await fetch(e),a=await t.blob();return new Promise((e,t)=>{let n=new FileReader;n.onloadend=()=>e(n.result),n.onerror=t,n.readAsDataURL(a)})}function y(e){let t=Math.abs(e);return`${e<0?"-":""}${t.toLocaleString("ko-KR")}`}function f(e,t){let a=e.getNumberOfPages(),n=e.internal.pageSize.getWidth(),i=e.internal.pageSize.getHeight();for(let o=1;o<=a;o++)e.setPage(o),e.setFontSize(7),(0,r.setKoreanFont)(e,"normal"),e.setTextColor(150,150,150),e.text(`OwnerView Document  |  ${t}  |  Page ${o}/${a}`,n/2,i-8,{align:"center"})}e.s(["generateContractPDF",()=>u,"generateDocumentPDF",()=>l,"generateQuotePDF",()=>d,"issueDocument",()=>c])},90001,e=>{"use strict";var t=e.i(7471),a=e.i(66154),n=e.i(59771),i=e.i(88617),r=e.i(87200);let o=t.supabase;async function s(e){let{companyId:a,dealId:n,docType:i,createdBy:r,items:s,paymentRatio:l}=e,{data:d,error:c}=await o.from("deals").select("*, partners!deals_partner_id_fkey(name, business_number, contact_email, contact_phone)").eq("id",n).eq("company_id",a).single();if(c||!d)throw Error(`딜을 찾을 수 없습니다 (id: ${n}, error: ${c?.message||"no data"})`);let u=d.partners?.name||d.counterparty||"",p=d.partners?.business_number||"",m=Number(d.contract_total||0),{data:y}=await o.from("companies").select("name, business_number, representative, address").eq("id",a).single(),f=y?.name||"",_=y?.business_number||"",g=y?.representative||"",b=y?.address||"",h={type:i,dealId:n,dealName:d.name,partnerName:u,partnerBizNo:p,partnerEmail:d.partners?.contact_email||"",partnerPhone:d.partners?.contact_phone||"",contractTotal:m,supplyAmount:m,taxAmount:Math.round(.1*m),totalWithTax:Math.round(1.1*m),sections:[],metadata:{autoGenerated:!0,sourceDocType:i,generatedAt:new Date().toISOString()}};if("contract"===i){let e=await (async()=>{let{data:e}=await o.from("documents").select("content_json").eq("deal_id",n).order("created_at",{ascending:!1});return(e||[]).find(e=>{let t=e.content_json;return t?.type==="invoice"||t?.type==="quote"})})(),t=e?.content_json,a=t?.items&&Array.isArray(t.items)&&t.items.length>0?t.items:[{name:d.name||"",quantity:1,unitPrice:m,supplyAmount:m,taxAmount:Math.round(.1*m),totalAmount:Math.round(1.1*m),note:""}],i=t?.paymentRatio?.advance??l?.advance??30,r=t?.paymentRatio?.balance??l?.balance??70;Object.assign(h,{contractStartDate:new Date().toISOString().split("T")[0],contractEndDate:"",paymentTerms:`계약금 ${i}% (계약 후 7일 이내), 잔금 ${r}% (납품 완료 후 14일 이내)`,paymentSchedule:[{label:"선금",ratio:i,amount:Math.round(m*i/100),condition:"계약 후 7일 이내"},{label:"잔금",ratio:r,amount:m-Math.round(m*i/100),condition:"납품 완료 후 14일 이내"}],paymentRatio:{advance:i,balance:r},items:a})}if("invoice"===i){let e=s&&s.length>0?s:d.items&&Array.isArray(d.items)&&d.items.length>0?d.items:[{name:d.name||"",quantity:1,unitPrice:m,supplyAmount:m,taxAmount:Math.round(.1*m),totalAmount:Math.round(1.1*m),note:""}];Object.assign(h,{validUntil:new Date(Date.now()+2592e6).toISOString().split("T")[0],items:e,...l?{paymentRatio:l}:{}})}let v=`${d.name} - ${{invoice:"견적서",contract:"계약서",quote:"제안서"}[i]}`,w=Math.round(.1*m),x=m+w,$=new Date().toISOString().split("T")[0],S=h.items||[];function I(e){let t="\n┌──────┬──────────────────────┬──────────┬──────────┬──────────────┬────────────────┐\n";return t+="│  No  │ 품명                 │ 수량     │ 단가     │ 공급가액     │ 비고           │\n├──────┼──────────────────────┼──────────┼──────────┼──────────────┼────────────────┤\n",e.forEach((e,a)=>{let n=String(a+1).padStart(4),i=(e.name||"").slice(0,18).padEnd(18),r=String(e.quantity||1).padStart(6),o=Number(e.unitPrice||0).toLocaleString("ko-KR").padStart(8),s=Number(e.supplyAmount||e.amount||0).toLocaleString("ko-KR").padStart(12),l=(e.note||"").slice(0,12).padEnd(12);t+=`│ ${n} │ ${i} │ ${r} │ ${o} │ ${s} │ ${l} │
...
${f}
대표이사 ${g} (직인)`}if("contract"===i){let e=(h.paymentSchedule||[]).map(e=>`  - ${e.label} (${e.ratio}%): ₩${e.amount.toLocaleString("ko-KR")} — ${e.condition}`).join("\n");h.body=`표 준 용 역 계 약 서
...
제5조 (계약기간)
${h.contractStartDate||$} ~ ${h.contractEndDate||"프로젝트 완료 시"}
