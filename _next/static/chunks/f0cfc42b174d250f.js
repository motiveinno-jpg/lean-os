...
[결제조건]
${v.paymentRatio?`선금 ${v.paymentRatio.advance}% / 잔금 ${v.paymentRatio.balance}%`:"협의"}

${m}
대표이사 ${f} (직인)`}if("contract"===i){let e=(v.paymentSchedule||[]).map(e=>`  - ${e.label} (${e.ratio}%): ₩${e.amount.toLocaleString("ko-KR")} — ${e.condition}`).join("\n");v.body=`표 준 용 역 계 약 서
...
제3조 (계약금액 및 부가가치세)
  공급가액:  ₩${p.toLocaleString("ko-KR")}
  부가가치세(10%): ₩${y.toLocaleString("ko-KR")}
  ─────────────────────
  합계금액:  ₩${_.toLocaleString("ko-KR")} (VAT 포함)

제4조 (대금지급)
${e||v.paymentTerms||"별도 협의"}

제5조 (계약기간)
${v.contractStartDate||$} ~ ${v.contractEndDate||"프로젝트 완료 시"}
