...
## 3-1. 꾸미기 — 마크다운이 네이버 부품으로 바뀐다
글이 줄글만 이어지면 지루하다(2026-09-04 사장님: "포스팅글도 좀 이쁘게"). 발행 스크립트가 아래처럼 바꿔 주니, **원고에서 이 문법을 적극 쓴다.**

| 원고(마크다운) | 네이버에서 |
|---|---|
| `## 소제목` | 위에 장식 구분선 + 브랜드색(#4F46E5) 굵은 큰 글씨 |
| `### 소제목` | 브랜드색 굵은 중간 글씨 |
| 첫 문단 | 조금 큰 글씨(리드) |
| `> 한 줄` | 따옴표 인용구(세로 막대) |
| 마크다운 표 | 네이버 표(머리행 굵게·브랜드색) |
| `![설명](/blog/…)` | 사진 + 아래 회색 작은 글씨 캡션(설명이 캡션이 된다) |
| `---` | 구분선 |
| 글 맨 끝 | 구분선 + 말풍선 한 마디 + 랜딩 링크(자동으로 붙는다) |

- **한 편에 표 1~2개, 인용구 1~2개**를 넣는다. 비교·정리는 줄글 대신 표로.
- 이미지 설명(alt)은 캡션으로 그대로 보이니 "재무 › 수집·전표 › 신용카드 탭"처럼 화면 이름을 적는다.
- 이모지·형광펜은 쓰지 않는다(광고처럼 보인다). 강조는 소제목·인용구·표로 충분하다.

## 3-2. 그림은 글이 말하는 그 화면이어야 한다 (2026-09-04 사장님 지적)
> "휴가 관련된 내용 글 밑에 이미지를 휴가관련된 이미지로 올려야지 그냥 근태관리 이미지 띡 올리고 이러면 안돼."

- **문장이 가리키는 것이 그림 안에 실제로 보여야 한다.** "연차·반차·지각이 쌓인다"고 썼으면 그 칸이 보이는 화면을, "숫자를 누르면 날짜가 나온다"고 썼으면 **누른 뒤 열린 팝업**을(캡처 항목의 `clickSelector`) 넣는다.
- 화면 하나로 두 가지를 말하지 않는다. 결재 이야기는 결재 허브 화면, 공지 이야기는 게시판 화면으로 **문단과 그림을 1:1**로 나눈다.
- 자료가 비어 있는 화면(전부 0·"—")을 두고 "쌓여요"라고 쓰지 않는다. 필요하면 `node scripts/blog-shot-seed.mjs` 로 QA 시드 회사 자료를 채우고 찍는다.
- 본문에서 숫자·이름을 말할 때는 **그림에 보이는 값 그대로** 쓴다("이준호 님은 지각 1회").
- 글을 올린 뒤 **그림마다 앞뒤 문단과 같이 한 번 읽어 본다.** 어긋나면 그림을 다시 찍거나 문장을 그림에 맞춘다.
