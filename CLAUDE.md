...
| `notion-agent` | **Notion QA 추적**. "모티브이노베이션_오너뷰" 페이지의 개선요청 완료 토글, 진행요약 갱신, 작업로그·추천 카드 append, 직원 원문 백업 보존. 코드 푸시 완료 후 QA 완료 표시, Notion 페이지 수정·재구성 요청. DB/표 금지(불릿 유지), 직원 원문 파괴 금지. |
...
5. **Notion QA 후처리** (코드 작업 푸시 완료 후)
   → 메인이 commit hash + 변경 파일 + 화면위치를 정리해 `notion-agent` 호출
   → notion-agent 가 해당 QA 항목 ✅ 완료 토글 + 진행요약 + 작업로그 카드 갱신
   → Notion 페이지 수정/추천/재구성 단독 요청도 `notion-agent` (코드 무관)
