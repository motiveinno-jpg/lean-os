...
</html>`}t.s(["generateAuditTrailCertificateHTML",()=>o,"getAuditTrail",()=>a,"logAuditTrail",()=>n],53051);let s=e.supabase;async function l(t){let e=new TextEncoder().encode(t);return Array.from(new Uint8Array(await crypto.subtle.digest("SHA-256",e))).map(t=>t.toString(16).padStart(2,"0")).join("")}async function c(t){let{data:e,error:r}=await s.from("hr_contract_package_items").select("id, sort_order, signature_data, documents(content_json)").eq("package_id",t).order("sort_order");if(r)throw Error(`패키지 아이템 조회 실패: ${r.message}`);if(!e||0===e.length)throw Error("패키지에 문서가 없습니다");let i=[];for(let t of e)t.documents?.content_json&&i.push(JSON.stringify(t.documents.content_json)),t.signature_data&&i.push(JSON.stringify(t.signature_data));return l(i.join("|"))}async function d(t,e){let{data:r,error:i}=await s.from("hr_contract_packages").select("notes").eq("id",t).single();if(i)throw Error(`패키지 조회 실패: ${i.message}`);let n={};if(r?.notes)try{n=JSON.parse(r.notes)}catch{n={text:r.notes}}n.document_hash=e,n.hash_generated_at=new Date().toISOString();let{error:a}=await s.from("hr_contract_packages").update({notes:JSON.stringify(n)}).eq("id",t);if(a)throw Error(`해시 저장 실패: ${a.message}`)}async function u(t){let{data:e,error:r}=await s.from("hr_contract_packages").select("notes").eq("id",t).single();if(r)throw Error(`패키지 조회 실패: ${r.message}`);let i="";if(e?.notes)try{i=JSON.parse(e.notes).document_hash||""}catch{}if(!i)throw Error("저장된 해시가 없습니다. 먼저 storeDocumentHash를 호출하세요.");let n=await c(t);return{valid:i===n,storedHash:i,currentHash:n}}t.s(["generatePackageHash",()=>c,"storeDocumentHash",()=>d,"verifyDocumentIntegrity",()=>u],53845)},84099,t=>{"use strict";var e=t.i(43476),r=t.i(71645),i=t.i(18566),n=t.i(7471),a=t.i(62164),o=t.i(53051),s=t.i(53845),l=t.i(87200);let c=n.supabase;function d(){return(0,e.jsx)(a.ToastProvider,{children:(0,e.jsx)(r.Suspense,{fallback:(0,e.jsx)("div",{className:"min-h-screen flex items-center justify-center bg-gray-50",children:(0,e.jsx)("div",{className:"w-8 h-8 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"})}),children:(0,e.jsx)(u,{})})})}function u(){let{toast:n}=(0,a.useToast)(),d=(0,i.useSearchParams)().get("token")||"",[u,f]=(0,r.useState)(!0),[p,h]=(0,r.useState)(!1),[m,g]=(0,r.useState)(null),[y,b]=(0,r.useState)(0),[x,v]=(0,r.useState)(null),[w,j]=(0,r.useState)(""),[N,_]=(0,r.useState)(!1),[S,k]=(0,r.useState)(!1),[A,$]=(0,r.useState)(null),[C,E]=(0,r.useState)(null),[I,T]=(0,r.useState)(!1),B=(0,r.useRef)(null),z=(0,r.useRef)(!1);async function O(){try{let{data:t}=await c.from("hr_contract_packages").select("*, employees(name, email, department, position), companies(name)").eq("sign_token",d).single();if(!t){let{data:t}=await c.from("signature_requests").select("*, documents(name, content_json, status, company_id)").eq("sign_token",d).single();if(t){let e=!!t.expires_at&&new Date(t.expires_at)<new Date,{data:r}=await c.from("companies").select("name").eq("id",t.documents?.company_id||t.company_id).single();g({id:t.id,title:t.title,status:t.status,expired:e,companies:r||{name:""},employees:{name:t.signer_name,email:t.signer_email,department:"",position:""},items:t.documents?[{id:t.id,title:t.documents.name||t.title,status:"signed"===t.status?"signed":"pending",documents:t.documents,sort_order:0}]:[],_isGeneralDoc:!0,_signatureRequestId:t.id}),"sent"===t.status&&await c.from("signature_requests").update({status:"viewed",viewed_at:new Date().toISOString()}).eq("id",t.id),f(!1);return}h(!0),f(!1);return}let e=!!t.expires_at&&new Date(t.expires_at)<new Date,{data:r}=await c.from("hr_contract_package_items").select("*, documents(name, content_json, status)").eq("package_id",t.id).order("sort_order");if(g({...t,expired:e,items:r||[]}),t.employee_id){let{data:e}=await c.from("employees").select("saved_signature").eq("id",t.employee_id).single();e?.saved_signature&&$(e.saved_signature)}"completed"===t.status&&k(!0);let i=(r||[]).findIndex(t=>"pending"===t.status);i>=0&&b(i),f(!1);try{(0,o.logAuditTrail)(t.id,{action:"document_opened",timestamp:new Date().toISOString(),actor:t.employees?.name||"unknown",userAgent:navigator.userAgent,details:`서명 페이지 접속`})}catch(t){console.error("Audit log error:",t)}}catch{h(!0),f(!1)}}(0,r.useEffect)(()=>{if(!d){h(!0),f(!1);return}O()},[d]);let R=(0,r.useCallback)(t=>{let e=B.current;if(!e)return;z.current=!0;let r=e.getContext("2d"),i=e.getBoundingClientRect(),n="touches"in t?t.touches[0].clientX-i.left:t.clientX-i.left,a="touches"in t?t.touches[0].clientY-i.top:t.clientY-i.top;r.beginPath(),r.moveTo(n,a)},[]),D=(0,r.useCallback)(t=>{if(!z.current)return;let e=B.current;if(!e)return;let r=e.getContext("2d"),i=e.getBoundingClientRect(),n="touches"in t?t.touches[0].clientX-i.left:t.clientX-i.left,a="touches"in t?t.touches[0].clientY-i.top:t.clientY-i.top;r.lineWidth=2,r.lineCap="round",r.strokeStyle="#1e293b",r.lineTo(n,a),r.stroke()},[]),U=(0,r.useCallback)(()=>{z.current=!1},[]),L=()=>{let t=B.current;t&&t.getContext("2d").clearRect(0,0,t.width,t.height)};async function P(){let e;if(!m)return;let r=m.items[y];if(r&&"signed"!==r.status){if("saved"===x&&A)e=A;else if("draw"===x){let t=B.current;if(!t)return;e={type:"draw",data:t.toDataURL("image/png")}}else{if("type"!==x||!w.trim())return;e={type:"type",data:w.trim()}}_(!0);try{let i=m._isGeneralDoc;if(i){let{saveSignature:r}=await t.A(27750);await r(m._signatureRequestId,e)}else await c.from("hr_contract_package_items").update({status:"signed",signed_at:new Date().toISOString(),signature_data:e}).eq("id",r.id);try{(0,o.logAuditTrail)(m.id,{action:"draw"===e.type?"signature_drawn":"signature_typed",timestamp:new Date().toISOString(),actor:m.employees?.name||"unknown",details:`서명 방식: ${"draw"===e.type?"직접 그리기":"텍스트 입력"}`})}catch(t){console.error("Audit log error:",t)}r.documents&&!i&&await c.from("documents").update({status:"locked",locked_at:new Date().toISOString()}).eq("id",r.document_id);let n=m.items.map((t,e)=>e===y?{...t,status:"signed",signed_at:new Date().toISOString()}:t),a=n.every(t=>"signed"===t.status),l=n.some(t=>"signed"===t.status);if(a&&!i){await c.from("hr_contract_packages").update({status:"completed",completed_at:new Date().toISOString()}).eq("id",m.id),k(!0);try{let t=await (0,s.generatePackageHash)(m.id);await (0,s.storeDocumentHash)(m.id,t)}catch(t){console.error("Hash generation error:",t)}try{await (0,o.logAuditTrail)(m.id,{action:"document_completed",timestamp:new Date().toISOString(),actor:m.employees?.name||"unknown",details:`전체 ${n.length}건 서명 완료`})}catch(t){console.error("Audit log error:",t)}try{let t="https://njbvdkuvtdtkxyylwngn.supabase.co",e=m.employees?.email||"",r=m.companies?.name||"";t&&e&&await fetch(`${t}/functions/v1/send-contract-email`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5qYnZka3V2dGR0a3h5eWx3bmduIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI1MjQyMDIsImV4cCI6MjA4ODEwMDIwMn0.Tcbxj-SP5814QEiaTBMi5SRjmB-ExRYV_b0zt_m9Kho"},body:JSON.stringify({to:e,employeeName:m.employees?.name||"",companyName:r,packageTitle:m.title,documentCount:n.length,signUrl:window.location.href,type:"completion",completedAt:new Date().toISOString()})})}catch(t){console.error("Completion email failed:",t)}}else l&&await c.from("hr_contract_packages").update({status:"partially_signed"}).eq("id",m.id);g({...m,items:n});let d=n.findIndex((t,e)=>e>y&&"pending"===t.status);d>=0&&(b(d),v(null),L(),j(""))}catch(t){n("서명 처리 중 오류: "+(t.message||"알 수 없는 오류"),"error")}finally{_(!1)}}}if(u)return(0,e.jsx)("div",{className:"min-h-screen flex items-center justify-center bg-gray-50",children:(0,e.jsxs)("div",{className:"text-center",children:[(0,e.jsx)("div",{className:"w-8 h-8 border-2 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"}),(0,e.jsx)("p",{className:"text-sm text-gray-500",children:"계약서를 불러오는 중..."})]})});if(p||!m)return(0,e.jsx)("div",{className:"min-h-screen flex items-center justify-center px-4 bg-gray-50",children:(0,e.jsxs)("div",{className:"w-full max-w-md text-center",children:[(0,e.jsx)("div",{className:"w-14 h-14 rounded-2xl bg-red-50 text-red-600 text-xl font-black flex items-center justify-center mx-auto mb-4",children:"!"}),(0,e.jsx)("h1",{className:"text-2xl font-extrabold text-gray-900 mb-2",children:"유효하지 않은 링크"}),(0,e.jsx)("p",{className:"text-gray-500 text-sm",children:"서명 링크가 만료되었거나 유효하지 않습니다. 담당자에게 문의해주세요."})]})});if(m.expired)return(0,e.jsx)("div",{className:"min-h-screen flex items-center justify-center px-4 bg-gray-50",children:(0,e.jsxs)("div",{className:"w-full max-w-md text-center",children:[(0,e.jsx)("div",{className:"w-14 h-14 rounded-2xl bg-yellow-50 text-yellow-600 text-xl font-black flex items-center justify-center mx-auto mb-4",children:"!"}),(0,e.jsx)("h1",{className:"text-2xl font-extrabold text-gray-900 mb-2",children:"서명 기한 만료"}),(0,e.jsx)("p",{className:"text-gray-500 text-sm",children:"서명 기한이 만료되었습니다. 회사 담당자에게 재발송을 요청해주세요."})]})});async function F(){if(m)try{let t=await (0,o.getAuditTrail)(m.id),e="N/A";if(m.notes)try{e=JSON.parse(m.notes).document_hash||"N/A"}catch{}try{let{data:t}=await c.from("hr_contract_packages").select("notes").eq("id",m.id).single();if(t?.notes){let r=JSON.parse(t.notes);r.document_hash&&(e=r.document_hash)}}catch{}let r=(0,o.generateAuditTrailCertificateHTML)({packageTitle:m.title,companyName:m.companies?.name||"",employeeName:m.employees?.name||"",signerEmail:m.employees?.email||"",documentNames:m.items.map(t=>t.title),auditEntries:t,documentHash:e}),i=window.open("","_blank");i&&(i.document.write(r),i.document.close())}catch(t){console.error("Audit trail error:",t),n("감사추적인증서를 불러오는 중 오류가 발생했습니다.","error")}}async function M(){if(m)try{let t=[];for(let e of m.items){let r=e.documents;if(!r?.content_json)continue;let i=r.content_json;if(i.title&&t.push(i.title),i.sections)for(let e of i.sections)e.heading&&t.push(`
${e.heading}`),e.body&&t.push(e.body);t.push("")}let e=await (0,l.generateDocumentPDF)({title:m.title,content:t.join("\n"),companyName:m.companies?.name||""}),r=URL.createObjectURL(e),i=document.createElement("a");i.href=r,i.download=`${m.title||"서명완료문서"}.pdf`,document.body.appendChild(i),i.click(),document.body.removeChild(i),URL.revokeObjectURL(r)}catch(t){console.error("PDF generation error:",t),n("PDF 생성 중 오류가 발생했습니다.","error")}}async function q(){if(m){T(!0);try{let t=await (0,s.verifyDocumentIntegrity)(m.id);E({valid:t.valid,hash:t.storedHash})}catch(t){console.error("Integrity check error:",t),E({valid:!1,hash:t.message||"검증 실패"})}finally{T(!1)}}}if(S)return(0,e.jsx)("div",{className:"min-h-screen flex items-center justify-center px-4 bg-gray-50",children:(0,e.jsxs)("div",{className:"w-full max-w-md",children:[(0,e.jsxs)("div",{className:"text-center",children:[(0,e.jsx)("div",{className:"w-16 h-16 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-4",children:(0,e.jsx)("svg",{className:"w-8 h-8 text-green-600",fill:"none",stroke:"currentColor",strokeWidth:"2.5",viewBox:"0 0 24 24",children:(0,e.jsx)("polyline",{points:"20 6 9 17 4 12"})})}),(0,e.jsx)("h1",{className:"text-2xl font-extrabold text-gray-900 mb-2",children:"서명 완료"}),(0,e.jsx)("p",{className:"text-gray-600 text-sm",children:"모든 문서에 서명이 완료되었습니다"}),(0,e.jsx)("p",{className:"text-gray-400 text-xs mt-1",children:"서명 완료 문서와 감사추적인증서가 이메일로 발송됩니다"})]}),(0,e.jsxs)("div",{className:"mt-6 p-4 bg-white rounded-xl border border-gray-200",children:[(0,e.jsx)("p",{className:"text-sm text-gray-600",children:m.title}),(0,e.jsxs)("p",{className:"text-xs text-gray-400 mt-1",children:["서명자: ",m.employees?.name," | 문서: ",m.items.length,"건"]})]}),(0,e.jsxs)("button",{onClick:F,className:"mt-4 w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition flex items-center justify-center gap-2",children:[(0,e.jsx)("svg",{className:"w-4 h-4",fill:"none",stroke:"currentColor",strokeWidth:"2",viewBox:"0 0 24 24",children:(0,e.jsx)("path",{d:"M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"})}),"감사추적인증서 보기"]}),(0,e.jsxs)("button",{onClick:M,className:"mt-3 w-full py-3 bg-gray-800 hover:bg-gray-900 text-white rounded-xl text-sm font-semibold transition flex items-center justify-center gap-2",children:[(0,e.jsx)("svg",{className:"w-4 h-4",fill:"none",stroke:"currentColor",strokeWidth:"2",viewBox:"0 0 24 24",children:(0,e.jsx)("path",{d:"M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"})}),"서명된 계약서 PDF 다운로드"]}),(0,e.jsxs)("div",{className:"mt-4 p-4 bg-white rounded-xl border border-gray-200",children:[(0,e.jsxs)("div",{className:"flex items-center justify-between",children:[(0,e.jsx)("p",{className:"text-sm font-semibold text-gray-700",children:"문서 무결성 검증"}),(0,e.jsx)("button",{onClick:q,disabled:I,className:"px-3 py-1.5 text-xs font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition disabled:opacity-50",children:I?"검증 중...":"검증하기"})]}),C&&(0,e.jsx)("div",{className:"mt-3",children:C.valid?(0,e.jsxs)("div",{className:"flex items-start gap-2 p-3 bg-green-50 rounded-lg border border-green-200",children:[(0,e.jsx)("span",{className:"text-green-600 mt-0.5",children:"✓"}),(0,e.jsxs)("div",{children:[(0,e.jsx)("p",{className:"text-sm font-medium text-green-700",children:"문서가 서명 후 변경되지 않았습니다"}),(0,e.jsxs)("p",{className:"text-xs text-green-600/70 mt-1 font-mono break-all",children:["SHA-256: ",C.hash]})]})]}):(0,e.jsxs)("div",{className:"flex items-start gap-2 p-3 bg-red-50 rounded-lg border border-red-200",children:[(0,e.jsx)("span",{className:"text-red-600 mt-0.5",children:"✗"}),(0,e.jsxs)("div",{children:[(0,e.jsx)("p",{className:"text-sm font-medium text-red-700",children:"문서가 변경된 것으로 감지됩니다"}),(0,e.jsx)("p",{className:"text-xs text-red-600/70 mt-1 font-mono break-all",children:C.hash})]})]})})]})]})});let K=m.items[y],W=m.items.filter(t=>"signed"===t.status).length,Y=K?.documents?.content_json;return(0,e.jsxs)("div",{className:"min-h-screen bg-gray-50",children:[(0,e.jsx)("header",{className:"bg-white border-b border-gray-200 sticky top-0 z-10",children:(0,e.jsxs)("div",{className:"max-w-3xl mx-auto px-4 py-3 flex items-center justify-between",children:[(0,e.jsxs)("div",{children:[(0,e.jsx)("h1",{className:"text-lg font-bold text-gray-900",children:m.title}),(0,e.jsxs)("p",{className:"text-xs text-gray-500",children:[m.employees?.name," (",m.employees?.department||"",")"]})]}),(0,e.jsx)("div",{className:"text-right",children:(0,e.jsxs)("span",{className:"inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700",children:[W,"/",m.items.length," 완료"]})})]})}),(0,e.jsx)("div",{className:"bg-white border-b border-gray-200",children:(0,e.jsx)("div",{className:"max-w-3xl mx-auto px-4 flex gap-1 overflow-x-auto py-2",children:m.items.map((t,r)=>(0,e.jsxs)("button",{onClick:()=>{b(r),v(null);try{(0,o.logAuditTrail)(m.id,{action:"document_viewed",timestamp:new Date().toISOString(),actor:m.employees?.name||"unknown",details:`문서 확인: ${t.title}`})}catch(t){console.error("Audit log error:",t)}},className:`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition ${r===y?"bg-blue-600 text-white":"signed"===t.status?"bg-green-50 text-green-700":"bg-gray-100 text-gray-600 hover:bg-gray-200"}`,children:["signed"===t.status&&"✓ ",t.title]},t.id))})}),(0,e.jsx)("div",{className:"max-w-3xl mx-auto px-4 py-6",children:K?.status==="signed"?(0,e.jsxs)("div",{className:"bg-white rounded-2xl border border-green-200 p-6 text-center",children:[(0,e.jsx)("div",{className:"w-12 h-12 rounded-full bg-green-100 flex items-center justify-center mx-auto mb-3",children:(0,e.jsx)("svg",{className:"w-6 h-6 text-green-600",fill:"none",stroke:"currentColor",strokeWidth:"2.5",viewBox:"0 0 24 24",children:(0,e.jsx)("polyline",{points:"20 6 9 17 4 12"})})}),(0,e.jsx)("p",{className:"text-green-700 font-semibold",children:"이 문서는 서명 완료되었습니다"}),(0,e.jsxs)("p",{className:"text-xs text-gray-400 mt-1",children:["서명 시각: ",K.signed_at?new Date(K.signed_at).toLocaleString("ko-KR"):"-"]})]}):(0,e.jsxs)(e.Fragment,{children:[(0,e.jsxs)("div",{className:"bg-white rounded-2xl border border-gray-200 p-6 md:p-8 mb-6 shadow-sm",children:[Y?.title&&(0,e.jsx)("h2",{className:"text-xl font-bold text-center text-gray-900 mb-6 pb-4 border-b border-gray-100",children:Y.title}),Y?.sections?.map((t,r)=>(0,e.jsxs)("div",{className:"mb-5",children:[t.heading&&(0,e.jsx)("h3",{className:"text-sm font-bold text-gray-800 mb-2",children:t.heading}),(0,e.jsx)("p",{className:"text-sm text-gray-600 leading-relaxed whitespace-pre-wrap",children:t.body})]},r))]}),(0,e.jsxs)("div",{className:"bg-white rounded-2xl border border-gray-200 p-6 shadow-sm",children:[(0,e.jsx)("h3",{className:"text-sm font-bold text-gray-800 mb-4",children:"서명"}),!x&&(0,e.jsxs)("div",{className:"space-y-3",children:[A&&(0,e.jsxs)("button",{onClick:()=>v("saved"),className:"w-full py-4 rounded-xl border-2 border-blue-200 bg-blue-50 hover:border-blue-400 transition text-center",children:[(0,e.jsxs)("div",{className:"flex items-center justify-center gap-2 mb-2",children:[(0,e.jsx)("svg",{className:"w-5 h-5 text-blue-600",fill:"none",stroke:"currentColor",strokeWidth:"2",viewBox:"0 0 24 24",children:(0,e.jsx)("polyline",{points:"20 6 9 17 4 12"})}),(0,e.jsx)("span",{className:"text-sm font-semibold text-blue-700",children:"저장된 서명 사용"})]}),"draw"===A.type?(0,e.jsx)("img",{src:A.data,alt:"저장된 서명",className:"h-12 mx-auto opacity-60"}):(0,e.jsx)("span",{className:"text-xl italic text-blue-800",style:{fontFamily:"cursive, serif"},children:A.data})]}),(0,e.jsxs)("div",{className:"flex gap-3",children:[(0,e.jsxs)("button",{onClick:()=>v("draw"),className:"flex-1 py-4 rounded-xl border-2 border-dashed border-gray-300 hover:border-blue-400 hover:bg-blue-50 transition text-center",children:[(0,e.jsx)("svg",{className:"w-6 h-6 mx-auto mb-1 text-gray-400",fill:"none",stroke:"currentColor",strokeWidth:"1.5",viewBox:"0 0 24 24",children:(0,e.jsx)("path",{d:"M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487z"})}),(0,e.jsx)("span",{className:"text-xs font-medium text-gray-600",children:"직접 그리기"})]}),(0,e.jsxs)("button",{onClick:()=>v("type"),className:"flex-1 py-4 rounded-xl border-2 border-dashed border-gray-300 hover:border-blue-400 hover:bg-blue-50 transition text-center",children:[(0,e.jsx)("svg",{className:"w-6 h-6 mx-auto mb-1 text-gray-400",fill:"none",stroke:"currentColor",strokeWidth:"1.5",viewBox:"0 0 24 24",children:(0,e.jsx)("path",{d:"M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 01.865-.501 48.172 48.172 0 003.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z"})}),(0,e.jsx)("span",{className:"text-xs font-medium text-gray-600",children:"텍스트 입력"})]})]})]}),"saved"===x&&A&&(0,e.jsxs)("div",{children:[(0,e.jsxs)("div",{className:"p-6 bg-gray-50 rounded-xl border-2 border-blue-200 text-center mb-4",children:[(0,e.jsx)("p",{className:"text-xs text-gray-500 mb-2",children:"저장된 서명"}),"draw"===A.type?(0,e.jsx)("img",{src:A.data,alt:"서명",className:"h-16 mx-auto"}):(0,e.jsx)("p",{className:"text-3xl italic text-gray-800",style:{fontFamily:"cursive, serif"},children:A.data})]}),(0,e.jsxs)("div",{className:"flex gap-2",children:[(0,e.jsx)("button",{onClick:()=>v(null),className:"px-4 py-2.5 text-sm rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50",children:"다른 방식"}),(0,e.jsx)("button",{onClick:P,disabled:N,className:"flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition disabled:opacity-50",children:N?"처리 중...":"서명 완료"})]})]}),"draw"===x&&(0,e.jsxs)("div",{children:[(0,e.jsxs)("div",{className:"relative border-2 border-gray-200 rounded-xl overflow-hidden mb-3",children:[(0,e.jsx)("canvas",{ref:B,width:600,height:200,className:"w-full h-[150px] cursor-crosshair touch-none bg-gray-50",onMouseDown:R,onMouseMove:D,onMouseUp:U,onMouseLeave:U,onTouchStart:R,onTouchMove:D,onTouchEnd:U}),(0,e.jsx)("button",{onClick:L,className:"absolute top-2 right-2 px-2 py-1 text-xs bg-white/80 hover:bg-white rounded border border-gray-200 text-gray-500",children:"지우기"})]}),(0,e.jsx)("p",{className:"text-xs text-gray-400 mb-4",children:"위 영역에 서명을 그려주세요"}),(0,e.jsxs)("div",{className:"flex gap-2",children:[(0,e.jsx)("button",{onClick:()=>{v(null),L()},className:"px-4 py-2.5 text-sm rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50",children:"취소"}),(0,e.jsx)("button",{onClick:P,disabled:N,className:"flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition disabled:opacity-50",children:N?"처리 중...":"서명 완료"})]})]}),"type"===x&&(0,e.jsxs)("div",{children:[(0,e.jsx)("input",{type:"text",value:w,onChange:t=>j(t.target.value),placeholder:"서명할 이름을 입력하세요",className:"w-full px-4 py-3 border-2 border-gray-200 rounded-xl text-lg text-center mb-3 focus:outline-none focus:border-blue-500",style:{fontFamily:"cursive, serif",fontSize:"24px"}}),(0,e.jsx)("p",{className:"text-xs text-gray-400 mb-4",children:"서명으로 사용할 이름을 입력하세요"}),(0,e.jsxs)("div",{className:"flex gap-2",children:[(0,e.jsx)("button",{onClick:()=>{v(null),j("")},className:"px-4 py-2.5 text-sm rounded-xl border border-gray-200 text-gray-600 hover:bg-gray-50",children:"취소"}),(0,e.jsx)("button",{onClick:P,disabled:N||!w.trim(),className:"flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition disabled:opacity-50",children:N?"처리 중...":"서명 완료"})]})]})]})]})}),(0,e.jsx)("footer",{className:"border-t border-gray-200 bg-white mt-8",children:(0,e.jsx)("div",{className:"max-w-3xl mx-auto px-4 py-4 text-center",children:(0,e.jsx)("p",{className:"text-xs text-gray-400",children:"OwnerView 전자서명 시스템"})})})]})}t.s(["default",()=>d])},48503,t=>{t.v(e=>Promise.all(["static/chunks/adabfc2d4bff09a9.js"].map(e=>t.l(e))).then(()=>e(15833)))},70653,t=>{t.v(e=>Promise.all(["static/chunks/049ce48f7172c019.js"].map(e=>t.l(e))).then(()=>e(24154)))},95111,t=>{t.v(e=>Promise.all(["static/chunks/dfff2fc9aec5c357.js"].map(e=>t.l(e))).then(()=>e(38201)))},27750,t=>{t.v(e=>Promise.all(["static/chunks/996cb79b27a62370.js"].map(e=>t.l(e))).then(()=>e(64130)))}]);